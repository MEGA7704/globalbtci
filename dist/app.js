...
function printA4(title,subtitle,body,orientation="portrait",options={}){
  const w=window.open("","_blank","width=1440,height=980");if(!w){toast("Autorisez les popups pour imprimer.",true);return}
  const c=S.session?.company||{},u=S.session?.user||{};
  const company=c.name||"GLOBAL BT",slogan=c.slogan||"La qualité, notre engagement";
  const phone=c.phone||u.phone||"—",mail=c.email||u.email||"—";
  const address=[c.address,c.city].filter(Boolean).join(", ")||"Côte d’Ivoire";
  const taxpayer=c.taxpayer_account||"—",rccm=c.rccm||"—",capital=Number(c.capital||0)>0?cash(c.capital):"—";
  const nowDate=new Date(),dateText=nowDate.toLocaleDateString("fr-FR"),stamp=nowDate.toLocaleString("fr-FR"),editor=u.full_name||"Administrateur";
  const period=nowDate.toLocaleDateString("fr-FR",{month:"long",year:"numeric"});
  const info=Array.isArray(options.info)?options.info:[];
  const infoMap=Object.fromEntries(info.map(x=>[String(x.label||"").trim().toLowerCase(),x.value]));
  const projectName=infoMap["projet"]||"";
  const projectCode=infoMap["code projet"]||"";
  const project=(S.data?.projects||[]).find(p=>(projectCode&&p.project_number===projectCode)||(projectName&&p.name===projectName));
  const projectState=project?statusLabel(project.status):"";
  const totalText=options.totalLabel&&options.total!==undefined?`${options.totalLabel} : ${options.total}`:`Document : ${title}`;
  const titleSub=subtitle||"Document officiel de gestion BTP";
  const portfolio=options.portfolio||(projectCode?projectCode:(titleSub||"Gestion BTP"));
  const summaryOneLabel=projectName?"Projet":"Entreprise / Client";
  const summaryOne=projectName||company;
  const summaryTwoLabel=projectCode?"Code projet":"Période";
  const summaryTwo=projectCode||period;
  const summaryThreeLabel=projectState?"Statut projet":"Statut global";
  const summaryThree=projectState||options.status||"Suivi en cours";
  const building=`<svg viewBox="0 0 92 92" aria-hidden="true"><path d="M10 78h70"/><path d="M18 78V34l18-12v56"/><path d="M38 78V18l15-9v69"/><path d="M55 78V43l16-9v44"/><path d="M18 35l18-12 2 4"/><path d="M53 17h22v4H58"/><path d="M72 20v29"/><path d="M71 33l9 5"/><path d="M26 45h6M26 55h6M26 65h6M44 33h5M44 44h5M44 55h5M61 51h5M61 61h5"/></svg>`;
  const icon=(name)=>({calendar:`<svg viewBox="0 0 24 24"><path d="M5 3v3M19 3v3M4 8h16M5 5h14a1 1 0 0 1 1 1v13H4V6a1 1 0 0 1 1-1zM8 12h2v2H8zm6 0h2v2h-2zM8 16h2v2H8z"/></svg>`,user:`<svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 21c.7-5 3.4-7 8-7s7.3 2 8 7"/></svg>`,brief:`<svg viewBox="0 0 24 24"><path d="M8 7V5h8v2M3 8h18v11H3zM3 12h18M10 12v2h4v-2"/></svg>`,chart:`<svg viewBox="0 0 24 24"><path d="M4 20h16M6 18v-5h3v5M11 18V8h3v10M16 18V4h3v14"/></svg>`,building:`<svg viewBox="0 0 24 24"><path d="M3 21h18M5 21V9l7-4v16M12 21V3l7 4v14M8 12h2M8 16h2M15 10h2M15 14h2M15 18h2"/></svg>`,shield:`<svg viewBox="0 0 24 24"><path d="M12 3l7 3v6c0 4.5-2.6 7.2-7 9-4.4-1.8-7-4.5-7-9V6zM8.5 12l2.3 2.3 4.8-5"/></svg>`}[name]||"");
  const metaCell=(ico,label,value)=>`<div class="meta-cell"><span class="meta-icon">${icon(ico)}</span><div><b>${esc(label)}</b><span>${esc(value??"—")}</span></div></div>`;
  const summaryCell=(ico,label,value)=>`<div class="summary-cell"><span class="summary-icon">${icon(ico)}</span><div><small>${esc(label)}</small><strong>${esc(value??"—")}</strong></div></div>`;
  w.document.write(`<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>${esc(title)}</title><style>
  @page{size:A4 ${orientation==="landscape"?"landscape":"portrait"};margin:4mm 5mm 6mm}
  *{box-sizing:border-box}html,body{margin:0;padding:0}body{font-family:"Segoe UI",Arial,Helvetica,sans-serif;color:#182f2b;background:#fff;font-size:${orientation==="landscape"?"8.9":"8.5"}px;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  .sheet{min-height:${orientation==="landscape"?"198mm":"285mm"};display:flex;flex-direction:column;background:#fff;padding:0 1.5mm}
  .company-head{display:grid;grid-template-columns:${orientation==="landscape"?"39% 32% 29%":"42% 30% 28%"};min-height:32mm;border-bottom:1.8px solid #063f37;align-items:stretch;padding:4mm 3mm 3mm}
  .brand-block{display:flex;align-items:center;gap:4mm;padding-right:4mm;border-right:1px solid #203b36}.brand-logo{width:${orientation==="landscape"?"27mm":"23mm"};height:${orientation==="landscape"?"27mm":"23mm"};display:grid;place-items:center;flex:0 0 auto}.brand-logo svg{width:100%;height:100%;fill:none;stroke:#063f37;stroke-width:3.3;stroke-linecap:round;stroke-linejoin:round}.brand-logo svg path:nth-child(4),.brand-logo svg path:nth-child(7),.brand-logo svg path:nth-child(8){stroke:#d19a28}
  .company-name{margin:0;color:#063f37;font-size:${orientation==="landscape"?"19":"15"}px;line-height:1.05;letter-spacing:.02em;font-weight:900}.company-sub{margin:2.5mm 0 0;color:#c68e22;font-size:${orientation==="landscape"?"10":"8.2"}px;font-weight:900}.company-slogan{margin:2.8mm 0 0;color:#27433d;font-size:${orientation==="landscape"?"8.4":"7.2"}px;font-style:italic}.brand-gold{width:30mm;height:1px;background:#c99327;margin-top:3mm}
  .contact-block{display:flex;flex-direction:column;justify-content:center;gap:2.2mm;padding:1mm 5mm;border-right:1px solid #203b36}.contact-row{display:grid;grid-template-columns:5mm 1fr;gap:2mm;align-items:center}.contact-row svg{width:4.6mm;height:4.6mm;fill:none;stroke:#06443b;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.contact-row b{color:#063f37}
  .legal-block{position:relative;display:flex;flex-direction:column;justify-content:center;gap:2.3mm;padding:1mm 5mm}.legal-block:after{content:"";position:absolute;right:0;top:2mm;bottom:2mm;width:1.4px;background:#c99327}.legal-row b{color:#063f37;font-weight:900}.legal-row span{color:#2e3d3a}
  .title-row{display:grid;grid-template-columns:1fr auto 1fr;gap:5mm;align-items:center;margin:7mm 8mm 3mm}.title-line{height:1px;background:#c99327;position:relative}.title-line.left:after,.title-line.right:before{content:"";position:absolute;width:2.2mm;height:2.2mm;background:#c99327;transform:rotate(45deg);top:-.9mm}.title-line.left:after{right:-.4mm}.title-line.right:before{left:-.4mm}.doc-title{text-align:center}.doc-title h1{margin:0;color:#063f37;font-size:${orientation==="landscape"?"22":"17"}px;letter-spacing:.04em;text-transform:uppercase;font-weight:950}.doc-title p{margin:1.4mm 0 0;color:#73807d;font-size:7.2px;font-weight:700}
  .meta-strip{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid #b8c4c1;border-radius:2mm;background:#fbfcfc;margin:0 3mm 5mm;overflow:hidden}.meta-cell{display:flex;align-items:center;gap:2.4mm;padding:3.1mm 4mm;min-height:13mm;position:relative}.meta-cell:not(:last-child):after{content:"";position:absolute;right:0;top:25%;bottom:25%;width:1px;background:#aeb9b6}.meta-icon{width:7.3mm;height:7.3mm;border:1.3px solid #0b5147;border-radius:50%;display:grid;place-items:center;flex:0 0 auto}.meta-icon svg{width:4.7mm;height:4.7mm;fill:none;stroke:#0a4b42;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}.meta-cell b{color:#123c35;margin-right:1mm}.meta-cell span{color:#344640}.meta-cell>div{display:flex;gap:1mm;align-items:baseline;flex-wrap:wrap}
  .summary-strip{display:grid;grid-template-columns:repeat(3,1fr);border:1.4px solid #c9962a;border-radius:2.2mm;margin:0 3mm 5mm;overflow:hidden}.summary-cell{display:flex;justify-content:center;align-items:center;gap:3mm;padding:3mm 4mm;min-height:15mm;position:relative}.summary-cell:not(:last-child):after{content:"";position:absolute;right:0;top:20%;bottom:20%;width:1px;background:#9b9f9d}.summary-icon{width:9mm;height:9mm;border-radius:50%;background:#06443b;display:grid;place-items:center;box-shadow:inset 0 0 0 1px #0b6a5d}.summary-icon svg{width:5.8mm;height:5.8mm;fill:none;stroke:#fff;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}.summary-cell small{display:block;color:#25423c;font-size:7px;font-weight:800;margin-bottom:.8mm}.summary-cell strong{display:block;color:#063f37;font-size:${orientation==="landscape"?"10":"8.4"}px}
  .content{flex:1;margin:0 3mm}.content h2,.content h3{color:#064f47}.content h3{font-size:10px;margin:4mm 0 1.5mm}.content table{margin-top:0}
  table{width:100%;border-collapse:separate;border-spacing:0;font-size:${orientation==="landscape"?"7.8":"7.6"}px;border:1px solid #9eaeaa;border-radius:1.8mm;overflow:hidden;table-layout:auto}th,td{padding:${orientation==="landscape"?"2.7":"2.4"}mm 2mm;vertical-align:middle;text-align:left;border-right:1px solid #b7c0be;border-bottom:1px solid #b7c0be}th:last-child,td:last-child{border-right:0}tbody tr:last-child td{border-bottom:0}th{background:#06483f;color:#fff;font-weight:900;text-align:center;font-size:7.2px;letter-spacing:.01em}tbody tr:nth-child(even){background:#fcfdfd}tr{break-inside:avoid}.money{text-align:right!important}.center{text-align:center!important}.nowrap{white-space:nowrap}
  .print-status{display:inline-block;padding:1.5mm 4mm;border-radius:3mm;font-weight:900;min-width:20mm;text-align:center;background:#e3f2e9;color:#154d3d}.print-status.planned{background:#ececed;color:#414141}.print-status.hold{background:#fae6d9;color:#8c4d20}.print-status.done{background:#dcefe5;color:#155c43}
  .print-spacer{min-height:${orientation==="landscape"?"5":"12"}mm}.total-row{display:flex;align-items:center;gap:2mm;margin:4mm 0 2mm;font-size:9px;font-weight:900;color:#133d36}.total-badge{width:7mm;height:7mm;border:1.2px solid #0a5146;border-radius:50%;display:grid;place-items:center}.total-badge svg{width:4.4mm;height:4.4mm;fill:none;stroke:#0a5146;stroke-width:1.7}
  .sign-grid{display:grid;grid-template-columns:1fr ${orientation==="landscape"?"36mm":"30mm"} 1fr;gap:8mm;align-items:end;margin:2mm 5mm 2mm}.signature h4{margin:0 0 2mm;color:#0b4139;font-size:8.5px;border-bottom:1px solid #c99327;width:max-content;padding-bottom:1mm}.sig-line{display:grid;grid-template-columns:auto 1fr;align-items:end;gap:2mm;margin:2.6mm 0;color:#263a36}.sig-line i{display:block;border-bottom:1px solid #4e5b58;height:4mm}.seal{width:${orientation==="landscape"?"34":"28"}mm;height:${orientation==="landscape"?"34":"28"}mm;border:1.4px solid #06483f;border-radius:50%;display:grid;place-items:center;position:relative;margin:auto;color:#06483f;text-align:center}.seal:before,.seal:after{content:"";position:absolute;border:1px solid #06483f;border-radius:50%;inset:2mm}.seal:after{inset:4mm;border-style:dashed}.seal-inner{position:relative;z-index:2;width:72%;font-weight:900;font-size:5.6px;line-height:1.15;text-transform:uppercase}.seal-inner svg{width:10mm;height:10mm;fill:none;stroke:#06483f;stroke-width:2.5}.seal-inner span{display:block;margin-top:1mm}.seal-star{font-size:7px;letter-spacing:1mm;color:#c99327}
  .page-no{display:flex;justify-content:center;align-items:center;gap:3mm;color:#0b4139;font-weight:900;margin:1mm 0 1.5mm}.page-no:before,.page-no:after{content:"";width:18mm;height:1px;background:#c99327}
  .footer-band{height:13mm;background:#063f37;color:#fff;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;margin:0 1mm;font-size:9px;font-weight:900;letter-spacing:.02em}.footer-band em{font-weight:600;margin-left:2mm}.footer-band:before,.footer-band:after{content:"";position:absolute;top:0;bottom:0;width:18mm;background:linear-gradient(120deg,#c38d22 0 24%,transparent 24% 35%,#d4a743 35% 55%,transparent 55% 66%,#c38d22 66%)}.footer-band:before{left:0}.footer-band:after{right:0;transform:scaleX(-1)}
  @media print{a{color:inherit;text-decoration:none}.sheet{padding:0}.sign-grid,.footer-band,.meta-strip,.summary-strip{break-inside:avoid}}
  </style></head><body><main class="sheet">
  <header class="company-head"><div class="brand-block"><div class="brand-logo">${building}</div><div><h2 class="company-name">${esc(company)}</h2><div class="company-sub">Gestion BTP Professionnelle</div><div class="company-slogan">${esc(slogan)}</div><div class="brand-gold"></div></div></div><div class="contact-block"><div class="contact-row">${icon("building")}<span>${esc(address)}</span></div><div class="contact-row"><svg viewBox="0 0 24 24"><path d="M5 4l4 4-2 3c2 3 4 5 7 7l3-2 3 4c-1 1-3 2-5 1C9 19 5 15 3 9 2 7 3 5 5 4z"/></svg><span>${esc(phone)}</span></div><div class="contact-row"><svg viewBox="0 0 24 24"><path d="M3 5h18v14H3zM3 7l9 7 9-7"/></svg><span>${esc(mail)}</span></div></div><div class="legal-block"><div class="legal-row"><b>N° Contribuable :</b> <span>${esc(taxpayer)}</span></div><div class="legal-row"><b>RCCM :</b> <span>${esc(rccm)}</span></div><div class="legal-row"><b>Capital :</b> <span>${esc(capital)}</span></div></div></header>
  <section class="title-row"><div class="title-line left"></div><div class="doc-title"><h1>${esc(title)}</h1><p>${esc(titleSub)}</p></div><div class="title-line right"></div></section>
  <section class="meta-strip">${metaCell("calendar","Date d’édition :",dateText)}${metaCell("user","Imprimé par :",editor)}${metaCell("brief","Portefeuille :",portfolio)}${metaCell("chart",options.totalLabel||"Document :",options.total!==undefined?String(options.total):title)}</section>
  <section class="summary-strip">${summaryCell("building",summaryOneLabel,summaryOne)}${summaryCell("calendar",summaryTwoLabel,summaryTwo)}${summaryCell("shield",summaryThreeLabel,summaryThree)}</section>
  <section class="content">${body}<div class="print-spacer"></div><div class="total-row"><span class="total-badge">${icon("chart")}</span><span>${esc(totalText)}</span></div></section>
  <section class="sign-grid"><div class="signature"><h4>Préparé par</h4><div class="sig-line"><span>Nom :</span><i></i></div><div class="sig-line"><span>Fonction :</span><i></i></div><div class="sig-line"><span>Date :</span><i></i></div></div><div class="seal"><div class="seal-inner">${building}<span>${esc(company)}</span><span>QUALITÉ<br>ENGAGEMENT<br>EXCELLENCE</span><span class="seal-star">★ ★</span></div></div><div class="signature"><h4>Approuvé par</h4><div class="sig-line"><span>Nom :</span><i></i></div><div class="sig-line"><span>Fonction :</span><i></i></div><div class="sig-line"><span>Date :</span><i></i></div></div></section>
  <div class="page-no">Page 1 / 1</div><footer class="footer-band"><span>${esc(company)} — <em>Gestion BTP Professionnelle</em></span></footer>
  ...
  $("#content").innerHTML=`
    ...
    <div class="panel dashboard-panel">
      <div class="panelhead">
        ...
        <button id="v41PrintDashboard" class="btn secondary">Imprimer / PDF</button>
      </div>
...
function reports(){const mat=S.data.expenses.reduce((a,x)=>a+Number(x.total_price),0),lab=S.data.labor.reduce((a,x)=>a+Number(x.amount),0);const by=S.data.trades.map(t=>({name:t.name,m:S.data.expenses.filter(x=>x.trade_id===t.id).reduce((a,x)=>a+Number(x.total_price),0),l:S.data.labor.filter(x=>x.trade_id===t.id).reduce((a,x)=>a+Number(x.amount),0)}));$("#content").innerHTML=`<div class="kpis">${kpi("Matériaux",cash(mat))}${kpi("Main-d'œuvre",cash(lab))}${kpi("Total",cash(mat+lab))}</div><div class="panel"><div class="panelhead"><h2>Bilan par métier</h2><button onclick="printA4('Bilan par métier','Rapport de gestion',this.closest('.panel').querySelector('.tablewrap')?.outerHTML||'', 'portrait')" class="btn secondary">Imprimer / PDF</button></div>${table(["Métier","Matériaux","Main-d'œuvre","Total"],by.map(x=>`<tr><td>${esc(x.name)}</td><td class="money">${cash(x.m)}</td><td class="money">${cash(x.l)}</td><td class="money"><strong>${cash(x.m+x.l)}</strong></td></tr>`))}</div>`}
...
function projects(){
  ...
  $('#content').innerHTML=`<div class="panel v41-projects-panel">
    <div class="panelhead"><div><h2>Liste des projets</h2><p class="muted">Chaque nouveau projet reçoit automatiquement un numéro unique au format PRJ-AAAA-001.</p></div><div class="toolbar"><button id="v41PrintProjects" class="btn secondary">Imprimer A4</button>${admin?'<button id="v28NewProject" class="btn primary">+ Nouveau projet</button>':''}</div></div>
