...
async function reload(){
  const raw=await api("/api/load");
  S.data=raw||{};
  const listKeys=["projects","trades","tradeCatalog","projectTrades","suppliers","projectSuppliers","expenses","labor","users","resets","companies","subscriptions","audit"];
  for(const k of listKeys){
    const v=S.data[k];
    if(Array.isArray(v))continue;
    if(Array.isArray(v?.results))S.data[k]=v.results;
    else S.data[k]=[];
  }
  return S.data;
}
...
function dashboard(){
  const p=Array.isArray(S.data?.projects)?S.data.projects:[];
  const e=Array.isArray(S.data?.expenses)?S.data.expenses:[];
  const l=Array.isArray(S.data?.labor)?S.data.labor:[];
  const tr=Array.isArray(S.data?.tradeCatalog)&&S.data.tradeCatalog.length?S.data.tradeCatalog:(Array.isArray(S.data?.trades)?S.data.trades:[]);
  ...
  const byTrade=tr.map(t=>{
    ...
    return {name:t.activity||t.name||"Métier",total:m+w,materials:m,labor:w};
...
document.addEventListener('submit',async e=>{
  if(e.target.id==='projectForm'){e.preventDefault();const form=e.target;try{await post('/api/save',{entity:'project',action:'create',record:fd(form)});closeModal();await reload();projects();toast('Projet créé')}catch(x){toast(x.message,true)}finally{releaseForm(form)}}
...
function renderProfessionalTradeForm(projectId=""){
  return `<form id="${projectId?"v37ProjectTradeForm":"v37TradeForm"}" ${projectId?`data-project="${esc(projectId)}"`:""}>
    ${projectId?"":`<label>Projet<select name="project_id" required>${opts(S.data.projects)}</select></label>`}
    <label>Corps principal<select id="v37TradeGroup" name="phase" required>${PROFESSIONAL_TRADE_GROUPS.map(g=>`<option value="${esc(g.key)}">${esc(g.label)}</option>`).join("")}</select></label>
    ...
    <label id="v37ActivityWrap">Activités<select id="v37Activity" name="name" required></select></label>
    ...
    <label>Description<textarea id="v37TradeDescription" name="description" rows="4"></textarea></label>
    <button class="btn primary full" type="submit">Ajouter le sous-corps de métier</button></form>`;
}
function initProfessionalTradeForm(root=document){
  const group=root.querySelector("#v37TradeGroup"),activity=root.querySelector("#v37Activity"),activityWrap=root.querySelector("#v37ActivityWrap"),customGroupWrap=root.querySelector("#v37CustomGroupWrap"),customGroup=root.querySelector("#v37CustomGroup"),customActivityWrap=root.querySelector("#v37CustomActivityWrap"),customActivity=root.querySelector("#v37CustomActivity"),desc=root.querySelector("#v37TradeDescription");
  if(!group)return;
  const refresh=()=>{const g=PROFESSIONAL_TRADE_GROUPS.find(x=>x.key===group.value)||PROFESSIONAL_TRADE_GROUPS[0],other=g.key==="autres";customGroupWrap.classList.toggle("hidden",!other);activityWrap.classList.toggle("hidden",other);customActivityWrap.classList.toggle("hidden",!other);if(other){activity.removeAttribute("required");desc.value="";return}activity.setAttribute("required","");activity.innerHTML=g.activities.map(([n,d])=>`<option value="${esc(n)}" data-description="${esc(d)}">${esc(n)}${d?` — ${esc(d)}`:""}</option>`).join("");customActivityWrap.classList.add("hidden");customActivity.value="";desc.value=g.activities[0]?.[1]||""};
  const changeActivity=()=>{if(activity.value==="Autre"){customActivityWrap.classList.remove("hidden");desc.value=""}else{customActivityWrap.classList.add("hidden");customActivity.value="";desc.value=activity.selectedOptions[0]?.dataset.description||""}};
  group.addEventListener("change",refresh);activity.addEventListener("change",changeActivity);refresh();
  root.addEventListener("submit",e=>{const f=e.target;if(!["v37TradeForm","v37ProjectTradeForm"].includes(f.id))return;const g=PROFESSIONAL_TRADE_GROUPS.find(x=>x.key===group.value);if(group.value==="autres"){if(!customGroup.value.trim()||!customActivity.value.trim()){e.preventDefault();e.stopImmediatePropagation();toast("Renseignez le corps principal et l'activité personnalisés.",true);releaseForm(f);return}f.dataset.resolvedPhase=customGroup.value.trim();f.dataset.resolvedName=customActivity.value.trim()}else if(activity.value==="Autre"){if(!customActivity.value.trim()){e.preventDefault();e.stopImmediatePropagation();toast("Renseignez l'activité personnalisée.",true);releaseForm(f);return}f.dataset.resolvedPhase=g.label;f.dataset.resolvedName=customActivity.value.trim()}else{f.dataset.resolvedPhase=g.label;f.dataset.resolvedName=activity.value}},true);
}

function trades(){
  const cat=S.data.tradeCatalog||[];
  $("#content").innerHTML=`<div class="panel"><div class="panelhead"><div><h2>Métiers</h2><p class="muted">Liste générale des métiers disponibles pour les projets.</p></div><div class="toolbar"><button id="v40PrintCatalog" class="btn secondary">Imprimer / PDF</button><button id="v40AddCatalog" class="btn primary">+ Ajouter un sous-corps de métier</button></div></div>${table(["Corps principal","Activité","Actions"],cat.map(x=>`<tr><td><strong>${esc(x.main_group)}</strong></td><td>${esc(x.activity)}</td><td class="center"><div class="actions centered"><button class="btn small secondary v40EditCatalog" data-id="${x.id}">Modifier</button>${S.session.user.role==="admin"?`<button class="btn small danger v40DeleteCatalog" data-id="${x.id}">Supprimer</button>`:""}</div></td></tr>`))}</div>`;
  $("#v40PrintCatalog").onclick=()=>printA4("Liste générale des métiers",S.session.company?.name||"",`<table><tr><th>Corps principal</th><th>Activité</th></tr>${cat.map(x=>`<tr><td>${esc(x.main_group)}</td><td>${esc(x.activity)}</td></tr>`).join("")}</table>`);
  $("#v40AddCatalog").onclick=()=>{modal(`<h2>Ajouter un sous-corps de métier</h2><form id="v40CatalogForm"><label>Corps principal<select id="v40Group">${PROFESSIONAL_TRADE_GROUPS.map(g=>`<option value="${esc(g.key)}">${esc(g.label)}</option>`).join("")}</select></label><label id="v40OtherGroupWrap" class="hidden">Autre corps principal<input id="v40OtherGroup"></label><label id="v40ActivityWrap">Activité<select id="v40Activity"></select></label><label id="v40OtherActivityWrap" class="hidden">Autre activité<input id="v40OtherActivity"></label><button class="btn primary full">Ajouter</button></form>`);v40InitCatalogForm()};
  document.querySelectorAll(".v40EditCatalog").forEach(b=>b.onclick=()=>{const x=cat.find(t=>t.id===b.dataset.id);modal(`<h2>Modifier le métier</h2><form id="v40CatalogEdit" data-id="${x.id}"><label>Corps principal<input name="main_group" value="${esc(x.main_group)}" required></label><label>Activité<input name="activity" value="${esc(x.activity)}" required></label><button class="btn primary full">Enregistrer</button></form>`)});
  document.querySelectorAll(".v40DeleteCatalog").forEach(b=>b.onclick=()=>confirmBox("Supprimer ce métier de la liste générale ?",async()=>{await post("/api/save",{entity:"trade_catalog",action:"delete",record:{id:b.dataset.id}});await reload();trades();toast("Métier supprimé")}));
}
function v40InitCatalogForm(){const g=$("#v40Group"),a=$("#v40Activity"),gw=$("#v40OtherGroupWrap"),aw=$("#v40ActivityWrap"),oaw=$("#v40OtherActivityWrap");const refresh=()=>{const x=PROFESSIONAL_TRADE_GROUPS.find(v=>v.key===g.value)||PROFESSIONAL_TRADE_GROUPS[0],other=x.key==="autres";gw.classList.toggle("hidden",!other);aw.classList.toggle("hidden",other);if(other){oaw.classList.remove("hidden");return}a.innerHTML=x.activities.map(([n])=>`<option value="${esc(n)}">${esc(n)}</option>`).join("");oaw.classList.toggle("hidden",a.value!=="Autre")};g.onchange=refresh;a.onchange=()=>oaw.classList.toggle("hidden",a.value!=="Autre");refresh()}
...
function render(){
  try{
    if(S.session.user.role==="superadmin")return renderSuper();
    const fn=({dashboard,projects,trades,suppliers,users,reports,settings}[S.view]||dashboard);
    return fn();
  }catch(err){
    console.error("GLOBAL_BT_RENDER_ERROR",err);
    const c=$("#content");
    if(c)c.innerHTML=`<div class="panel"><div class="panelhead"><div><h2>Affichage temporairement indisponible</h2><p class="muted">Une donnée ancienne ou incomplète a été détectée.</p></div></div><p><strong>Détail :</strong> ${esc(err?.message||String(err))}</p><button class="btn primary" id="retryRender">Réessayer</button></div>`;
    $("#retryRender")?.addEventListener("click",async()=>{try{await reload();render()}catch(e){toast(e.message,true)}});
  }
}
...
function v40ProjectPage(projectId,initialView="trades"){
  const p=S.data.projects.find(x=>x.id===projectId);if(!p)return;S.currentProjectId=projectId;
  const links=(S.data.projectTrades||[]).filter(x=>x.project_id===projectId),cat=S.data.tradeCatalog||[],materials=(S.data.expenses||[]).filter(x=>x.project_id===projectId),suppliers=(S.data.projectSuppliers||[]).filter(x=>x.project_id===projectId),labor=(S.data.labor||[]).filter(x=>x.project_id===projectId);
  const matTotal=materials.reduce((a,x)=>a+Number(x.total_price||0),0),labTotal=labor.reduce((a,x)=>a+Number(x.amount||0),0);
  $("#content").innerHTML=`<section class="project-page-pro"><div class="project-page-toolbar"><button id="projectBack" class="btn secondary">← Retour aux projets</button><span class="eyebrow">ESPACE PROJET</span></div><div class="project-page-hero"><div class="project-page-identity"><div class="project-page-icon">▥</div><div><h1>${esc(p.name)}</h1><p>⌖ ${esc(p.location||"")}</p><span class="status">${esc(p.status||"")}</span></div></div><div class="project-page-kpis"><div class="project-page-kpi"><small>Budget</small><strong>${cash(p.budget)}</strong></div><div class="project-page-kpi gold"><small>Matériaux</small><strong>${cash(matTotal)}</strong></div><div class="project-page-kpi blue"><small>Main-d'œuvre</small><strong>${cash(labTotal)}</strong></div></div></div><div class="project-page-tabs"><button class="project-page-tab" data-view="trades">Métiers <b>${links.length}</b></button><button class="project-page-tab" data-view="expenses">Matériaux <b>${materials.length}</b></button><button class="project-page-tab" data-view="suppliers">Fournisseurs <b>${suppliers.length}</b></button></div><div id="projectPageContent"></div></section>`;
  $("#projectBack").onclick=()=>projects();const tabs=[...document.querySelectorAll(".project-page-tab")],content=$("#projectPageContent");
  function show(view){tabs.forEach(b=>b.classList.toggle("active",b.dataset.view===view));
    if(view==="trades"){
      content.innerHTML=`<div class="project-page-section-head"><div><h2>Métiers du projet</h2><p>Un métier doit être enregistré au préalable dans la liste générale.</p></div><div class="toolbar"><button id="v40PrintProjectTrades" class="btn secondary">Imprimer / PDF</button><button id="v40AddProjectTrade" class="btn primary">+ Ajouter au projet</button></div></div><div class="project-page-card">${table(["Métier / Corps principal","Activité","Description activité","Main-d'œuvre","Actions"],links.map(x=>{const total=labor.filter(l=>l.trade_id===x.trade_catalog_id).reduce((a,l)=>a+Number(l.amount||0),0);return `<tr><td><strong>${esc(x.main_group)}</strong></td><td>${esc(x.activity)}</td><td>${esc(x.activity_description||"—")}</td><td class="money">${cash(total)}</td><td class="center"><div class="actions centered"><button class="btn small secondary v40EditProjectTrade" data-id="${x.id}">Modifier</button><button class="btn small danger v40RemoveProjectTrade" data-id="${x.id}">Supprimer</button></div></td></tr>`}))}</div>`;
      $("#v40AddProjectTrade").onclick=()=>{const used=new Set(links.map(x=>x.trade_catalog_id)),avail=cat.filter(x=>!used.has(x.id));modal(`<h2>Ajouter un métier au projet</h2><form id="v40ProjectTradeForm" data-project="${projectId}"><label>Métier enregistré<select name="trade_catalog_id" required>${avail.map(x=>`<option value="${x.id}">${esc(x.main_group)} — ${esc(x.activity)}</option>`).join("")}</select></label><button class="btn primary full">Ajouter</button></form>`)};
      document.querySelectorAll(".v40EditProjectTrade").forEach(b=>b.onclick=()=>{const x=links.find(v=>v.id===b.dataset.id);modal(`<h2>Métier du projet</h2><p><strong>${esc(x.main_group)}</strong></p><p>${esc(x.activity)}</p><p>${esc(x.activity_description||"")}</p><p class="muted">La définition se modifie dans la liste générale des métiers.</p><button class="btn primary full" onclick="closeModal()">Fermer</button>`)});
      document.querySelectorAll(".v40RemoveProjectTrade").forEach(b=>b.onclick=()=>confirmBox("Retirer ce métier du projet ?",async()=>{await post("/api/save",{entity:"project_trade",action:"delete",record:{id:b.dataset.id,project_id:projectId}});await reload();v40ProjectPage(projectId,"trades");toast("Métier retiré du projet")}));
      $("#v40PrintProjectTrades").onclick=()=>printA4(`Liste des métiers — ${p.name}`,p.location||"",`<table><tr><th>Corps principal</th><th>Activité</th><th>Description activité</th><th>Main-d'œuvre</th></tr>${links.map(x=>{const total=labor.filter(l=>l.trade_id===x.trade_catalog_id).reduce((a,l)=>a+Number(l.amount||0),0);return `<tr><td>${esc(x.main_group)}</td><td>${esc(x.activity)}</td><td>${esc(x.activity_description||"")}</td><td>${cash(total)}</td></tr>`}).join("")}</table>`);
    }
    if(view==="expenses"){
      content.innerHTML=`<div class="project-page-section-head"><div><h2>Matériaux du projet</h2><p>Recherche par métier, date ou fournisseur.</p></div><div class="toolbar"><button id="v40PrintMaterials" class="btn secondary">Imprimer / PDF</button><button id="v40AddMaterial" class="btn primary">+ Ajouter des matériaux</button></div></div><div class="project-searchbar"><input id="v40MaterialSearch" type="search" placeholder="Rechercher par métier, date ou fournisseur..."></div><div id="v40MaterialTable" class="project-page-card"></div>`;
      const draw=()=>{const q=$("#v40MaterialSearch").value.trim().toLowerCase(),rows=materials.filter(x=>!q||`${x.trade_main_group||""} ${x.trade_activity||x.trade_name||""}`.toLowerCase().includes(q)||String(x.expense_date||"").toLowerCase().includes(q)||String(x.supplier_name||"").toLowerCase().includes(q));$("#v40MaterialTable").innerHTML=table(["Date","Métier / Corps principal","Activité","Désignation","Fournisseur","Quantité","Prix unitaire","Prix total","Actions"],rows.map(x=>`<tr><td>${df(x.expense_date)}</td><td>${esc(x.trade_main_group||"—")}</td><td>${esc(x.trade_activity||x.trade_name||"—")}</td><td>${esc(x.description||"")}</td><td>${esc(x.supplier_name||"—")}</td><td>${Number(x.quantity||0)}</td><td class="money">${cash(x.unit_price)}</td><td class="money">${cash(x.total_price)}</td><td class="center"><div class="actions centered"><button class="btn small secondary v40EditMaterial" data-id="${x.id}">Modifier</button><button class="btn small danger v40DeleteMaterial" data-id="${x.id}">Supprimer</button></div></td></tr>`));document.querySelectorAll(".v40EditMaterial").forEach(b=>b.onclick=()=>{const x=materials.find(v=>v.id===b.dataset.id);modal(`<h2>Modifier les matériaux</h2><form id="v40MaterialEdit" data-id="${x.id}" data-project="${projectId}"><label>Date<input name="expense_date" type="date" value="${esc(x.expense_date||"")}"></label><label>Métier<select name="trade_id">${links.map(l=>`<option value="${l.trade_catalog_id}" ${l.trade_catalog_id===x.trade_id?"selected":""}>${esc(l.main_group)} — ${esc(l.activity)}</option>`).join("")}</select></label><label>Fournisseur<select name="supplier_id">${opts(S.data.suppliers||[],x.supplier_id)}</select></label><label>Désignation<input name="description" value="${esc(x.description||"")}" required></label><label>Quantité<input name="quantity" type="number" step=".01" value="${Number(x.quantity||0)}"></label><label>Prix unitaire<input name="unit_price" type="number" value="${Number(x.unit_price||0)}"></label><button class="btn primary full">Enregistrer</button></form>`)});document.querySelectorAll(".v40DeleteMaterial").forEach(b=>b.onclick=()=>confirmBox("Supprimer ces matériaux ?",async()=>{await post("/api/save",{entity:"expense",action:"delete",record:{id:b.dataset.id}});await reload();v40ProjectPage(projectId,"expenses");toast("Matériaux supprimés")}))};$("#v40MaterialSearch").oninput=draw;draw();
      $("#v40AddMaterial").onclick=()=>modal(`<h2>Ajouter des matériaux</h2><form id="v40MaterialForm" data-project="${projectId}"><label>Date<input name="expense_date" type="date" value="${new Date().toISOString().slice(0,10)}"></label><label>Métier<select name="trade_id" required>${links.map(l=>`<option value="${l.trade_catalog_id}">${esc(l.main_group)} — ${esc(l.activity)}</option>`).join("")}</select></label><label>Fournisseur<select name="supplier_id">${opts(S.data.suppliers||[])}</select></label><label>Désignation<input name="description" required></label><label>Quantité<input name="quantity" type="number" step=".01" value="1"></label><label>Prix unitaire<input name="unit_price" type="number" min="0"></label><button class="btn primary full">Enregistrer</button></form>`);
      $("#v40PrintMaterials").onclick=()=>printA4(`Liste des matériaux — ${p.name}`,p.location||"",`<table><tr><th>Date</th><th>Corps principal</th><th>Activité</th><th>Désignation</th><th>Fournisseur</th><th>Qté</th><th>PU</th><th>Total</th></tr>${materials.map(x=>`<tr><td>${df(x.expense_date)}</td><td>${esc(x.trade_main_group||"")}</td><td>${esc(x.trade_activity||x.trade_name||"")}</td><td>${esc(x.description||"")}</td><td>${esc(x.supplier_name||"")}</td><td>${Number(x.quantity||0)}</td><td>${cash(x.unit_price)}</td><td>${cash(x.total_price)}</td></tr>`).join("")}</table>`);
    }
    if(view==="suppliers"){
      content.innerHTML=`<div class="project-page-section-head"><div><h2>Fournisseurs du projet</h2></div><div class="toolbar"><button id="v40PrintProjectSuppliers" class="btn secondary">Imprimer / PDF</button><button id="v40AddProjectSupplier" class="btn primary">+ Affecter un fournisseur</button></div></div><div class="project-page-card">${table(["Fournisseur","Contact","Spécialité","Actions"],suppliers.map(x=>`<tr><td><strong>${esc(x.supplier_name)}</strong></td><td>${esc(x.phone||"")}</td><td>${esc(x.specialty||"")}</td><td class="center"><button class="btn small danger v40RemoveSupplier" data-id="${x.id}">Supprimer</button></td></tr>`))}</div>`;
      $("#v40AddProjectSupplier").onclick=()=>modal(`<h2>Affecter un fournisseur</h2><form id="v40ProjectSupplierForm" data-project="${projectId}"><label>Fournisseur<select name="supplier_id" required>${opts(S.data.suppliers||[])}</select></label><button class="btn primary full">Affecter</button></form>`);document.querySelectorAll(".v40RemoveSupplier").forEach(b=>b.onclick=()=>confirmBox("Retirer ce fournisseur du projet ?",async()=>{await post("/api/save",{entity:"project_supplier",action:"delete",record:{id:b.dataset.id,project_id:projectId}});await reload();v40ProjectPage(projectId,"suppliers");toast("Fournisseur retiré")}));$("#v40PrintProjectSuppliers").onclick=()=>printA4(`Liste des fournisseurs — ${p.name}`,p.location||"",`<table><tr><th>Fournisseur</th><th>Contact</th><th>Spécialité</th></tr>${suppliers.map(x=>`<tr><td>${esc(x.supplier_name)}</td><td>${esc(x.phone||"")}</td><td>${esc(x.specialty||"")}</td></tr>`).join("")}</table>`);
    }
...
function projects(){
  ...
  document.querySelectorAll('.v28-project-row').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;v40ProjectPage(r.dataset.id)});
...
document.addEventListener('submit',async e=>{const f=e.target;if(!f.id?.startsWith('v28'))return;e.preventDefault();try{if(f.id==='v28ProjectCreate'){await post('/api/save',{entity:'project',action:'create',record:fd(f)});closeModal();await reload();projects();toast('Projet créé')}else if(f.id==='v28ProjectEdit'){await post('/api/save',{entity:'project',action:'update',record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();projects();toast('Projet modifié')}else if(f.id==='v28StatusForm'){await post('/api/save',{entity:'project',action:'set_status',record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();projects();toast('Statut mis à jour')}else if(f.id==='v28TradeForm'){await post('/api/save',{entity:'trade',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v40ProjectPage(f.dataset.project);toast('Métier ajouté')}else if(f.id==='v28ExpenseForm'){await post('/api/save',{entity:'expense',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v40ProjectPage(f.dataset.project);toast('Matériaux enregistrés')}else if(f.id==='v28SupplierForm'){await post('/api/save',{entity:'project_supplier',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v40ProjectPage(f.dataset.project);toast('Fournisseur affecté')}}catch(x){toast(x.message,true)}finally{releaseForm(f)}},true);
...
document.addEventListener("submit",async e=>{
  ...
  try{
    if(f.id==="v29TradeForm"){
      ...
      await reload();closeModal();v40ProjectPage(f.dataset.project);toast("Métier ajouté");
      ...
      await reload();closeModal();v40ProjectPage(f.dataset.project);toast("Métier modifié");
    }else if(f.id==="v29ExpenseForm"){
      await post("/api/save",{entity:"expense",action:"create",record:{project_id:f.dataset.project,...fd(f)}});
      await reload();closeModal();v40ProjectPage(f.dataset.project);toast("Matériaux enregistrés");
      ...
      await reload();closeModal();v40ProjectPage(f.dataset.project);toast("Fournisseur affecté");
    }
...
document.addEventListener("submit",async e=>{
  ...
  try{
    if(f.id==="v36TradeForm"){await post("/api/save",{entity:"trade",action:"create",record:{project_id:f.dataset.project,...fd(f)}});closeModal();await reload();v40ProjectPage(f.dataset.project,"trades");toast("Métier ajouté")}
    else if(f.id==="v36TradeEditForm"){await post("/api/save",{entity:"trade",action:"update",record:{id:f.dataset.id,project_id:f.dataset.project,...fd(f)}});closeModal();await reload();v40ProjectPage(f.dataset.project,"trades");toast("Métier modifié")}
    else if(f.id==="v36ExpenseForm"){await post("/api/save",{entity:"expense",action:"create",record:{project_id:f.dataset.project,...fd(f)}});closeModal();await reload();v40ProjectPage(f.dataset.project,"expenses");toast("Matériaux enregistrés")}
    else if(f.id==="v36SupplierForm"){await post("/api/save",{entity:"project_supplier",action:"create",record:{project_id:f.dataset.project,...fd(f)}});closeModal();await reload();v40ProjectPage(f.dataset.project,"suppliers");toast("Fournisseur affecté")}
  }catch(x){toast(x.message,true)}
  finally{releaseForm(f)}
},true);
...
document.addEventListener("click",e=>{
  ...
  if(e.target?.id==="v37PrintProjectTrades"){const rows=(S.data.trades||[]).filter(x=>x.project_id===pid);printA4(`Métiers · ${p.name}`,p.location||"",`<table><tr><th>Corps principal</th><th>Activité</th><th>Description</th></tr>${rows.map(x=>`<tr><td>${esc(x.phase||"")}</td><td>${esc(x.name)}</td><td>${esc(x.description||"")}</td></tr>`).join("")}</table>`)}
...
document.addEventListener("submit",async e=>{const f=e.target;if(!["v37TradeForm","v37ProjectTradeForm","v37EditTradeForm"].includes(f.id))return;e.preventDefault();try{if(f.id==="v37EditTradeForm"){await post("/api/save",{entity:"trade",action:"update",record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();trades();toast("Métier modifié");return}const record=fd(f);record.phase=f.dataset.resolvedPhase||record.phase;record.name=f.dataset.resolvedName||record.name;if(f.id==="v37ProjectTradeForm")record.project_id=f.dataset.project;await post("/api/save",{entity:"trade",action:"create",record});closeModal();await reload();if(f.id==="v37ProjectTradeForm")v40ProjectPage(f.dataset.project,"trades");else trades();toast("Sous-corps de métier ajouté")}catch(x){toast(x.message,true)}finally{releaseForm(f)}},true);

document.addEventListener("submit",async e=>{const f=e.target;if(!f.id?.startsWith("v40"))return;e.preventDefault();try{
if(f.id==="v40CatalogForm"){const g=$("#v40Group"),a=$("#v40Activity"),def=PROFESSIONAL_TRADE_GROUPS.find(x=>x.key===g.value);let main=def?.label||g.value,act=a?.value||"",desc="";if(g.value==="autres"){main=$("#v40OtherGroup").value.trim();act=$("#v40OtherActivity").value.trim()}else if(act==="Autre")act=$("#v40OtherActivity").value.trim();else desc=def?.activities.find(x=>x[0]===act)?.[1]||"";if(!main||!act)throw new Error("Corps principal et activité obligatoires");await post("/api/save",{entity:"trade_catalog",action:"create",record:{main_group:main,activity:act,activity_description:desc}});closeModal();await reload();trades();toast("Métier ajouté")}
else if(f.id==="v40CatalogEdit"){await post("/api/save",{entity:"trade_catalog",action:"update",record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();trades();toast("Métier modifié")}
else if(f.id==="v40ProjectTradeForm"){await post("/api/save",{entity:"project_trade",action:"create",record:{project_id:f.dataset.project,trade_catalog_id:f.trade_catalog_id.value}});closeModal();await reload();v40ProjectPage(f.dataset.project,"trades");toast("Métier ajouté au projet")}
else if(f.id==="v40MaterialForm"){await post("/api/save",{entity:"expense",action:"create",record:{project_id:f.dataset.project,...fd(f)}});closeModal();await reload();v40ProjectPage(f.dataset.project,"expenses");toast("Matériaux ajoutés")}
else if(f.id==="v40MaterialEdit"){await post("/api/save",{entity:"expense",action:"update",record:{id:f.dataset.id,project_id:f.dataset.project,...fd(f)}});closeModal();await reload();v40ProjectPage(f.dataset.project,"expenses");toast("Matériaux modifiés")}
else if(f.id==="v40ProjectSupplierForm"){await post("/api/save",{entity:"project_supplier",action:"create",record:{project_id:f.dataset.project,supplier_id:f.supplier_id.value}});closeModal();await reload();v40ProjectPage(f.dataset.project,"suppliers");toast("Fournisseur affecté")}
}catch(x){toast(x.message,true)}finally{releaseForm(f)}},true);
window.addEventListener("error",e=>console.error("GLOBAL_BT_WINDOW_ERROR",e.error||e.message));
window.addEventListener("unhandledrejection",e=>console.error("GLOBAL_BT_PROMISE_ERROR",e.reason));
