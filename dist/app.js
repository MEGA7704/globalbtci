...
function settings(){const plan=S.session.company.plan;const actions=plan==="free"?`<div class="promoactions"><a class="btn secondary" target="_blank" rel="noopener" href="${esc(S.session.standardPaymentUrl)}">Standard · 2 100 FCFA</a><a class="btn gold" target="_blank" rel="noopener" href="${esc(S.session.businessPaymentUrl)}">Business · 20 600 FCFA</a></div>`:plan==="standard"?`<div class="promoactions"><a class="btn gold" target="_blank" rel="noopener" href="${esc(S.session.businessPaymentUrl)}">Passer à Business · 20 600 FCFA</a></div>`:`<div class="notice">Plan Business actif · accès complet 365 jours.</div>`;$("#content").innerHTML=`<div class="grid2"><div class="panel"><h2>Mon compte</h2><p>${esc(S.session.user.full_name)}<br>${esc(S.session.user.email)}</p><button id="changePwd" class="btn primary">Changer le mot de passe</button></div><div class="panel"><h2>Abonnement</h2><p>Plan : <strong>${esc(plan.toUpperCase())}</strong><br>Expiration : <strong>${df(S.session.company.plan_expires_at)}</strong></p><div class="notice">Free : 10 jours · 0 FCFA<br>Standard : 30 jours · 2 100 FCFA<br>Business : 365 jours · 20 600 FCFA</div>${actions}</div></div>`;$("#changePwd").onclick=changePassword}
...
function promo(){modal(`<div class="promo"><h2>Choisissez votre abonnement GLOBAL BT</h2><p>Votre Plan Free donne un accès complet pendant <strong>10 jours</strong>.</p><div class="notice">Free : accès complet 10 jours · <strong>0 FCFA</strong><br>Standard : accès complet 30 jours · <strong>2 100 FCFA</strong><br>Business : accès complet 365 jours · <strong>20 600 FCFA</strong></div><div class="promoactions"><button id="promoOk" class="btn secondary">Continuer en Free</button><a class="btn secondary" target="_blank" rel="noopener" href="${esc(S.session.standardPaymentUrl)}">Payer Standard · 2 100 FCFA</a><a class="btn gold" target="_blank" rel="noopener" href="${esc(S.session.businessPaymentUrl)}">Payer Business · 20 600 FCFA</a></div></div>`);$("#promoOk").onclick=closeModal;S.promo=Date.now()}
...
function superDash(){const c=S.data.companies||[],u=S.data.users||[],r=S.data.resets||[];$("#content").innerHTML=`<div class="kpis">${kpi("Entreprises",c.length)}${kpi("Membres",u.length)}${kpi("Free",c.filter(x=>x.plan==="free").length)}${kpi("Standard",c.filter(x=>x.plan==="standard").length)}${kpi("Business",c.filter(x=>x.plan==="business").length)}${kpi("Demandes mot de passe",r.filter(x=>x.status==="pending").length)}</div><div class="panel"><h2>Super Administration GLOBAL BT</h2><p>Gestion centrale des entreprises, membres, abonnements et actions sensibles.</p></div>`}
...
function superSubscriptions(){$("#content").innerHTML=`<div class="panel"><h2>Abonnements</h2><div class="notice">Free : 10 jours · 0 FCFA &nbsp;|&nbsp; Standard : 30 jours · 2 100 FCFA &nbsp;|&nbsp; Business : 365 jours · 20 600 FCFA.</div>${table(["Entreprise","Plan","Début","Expiration","État"],S.data.companies.map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(x.plan)}</td><td>${df(x.plan_started_at)}</td><td>${df(x.plan_expires_at)}</td><td>${Date.parse(x.plan_expires_at)>Date.now()?"Valide":"Expiré"}</td></tr>`))}</div>`}
