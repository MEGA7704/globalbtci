...
`CREATE TABLE IF NOT EXISTS app_meta(key TEXT PRIMARY KEY,value TEXT,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`
  ...
  await ensureColumn(env,"projects","locked","INTEGER NOT NULL DEFAULT 0");
  ...
  await ensureColumn(env,"expenses","trade_phase_snapshot","TEXT");
  await ensureColumn(env,"expenses","trade_name_snapshot","TEXT");
  ...
  // V40 : demandes d'activation d'abonnement envoyées au support / Super Admin.
  await ensureColumn(env,"subscription_activation_requests","company_id","TEXT");
  await ensureColumn(env,"subscription_activation_requests","requested_by","TEXT");
  await ensureColumn(env,"subscription_activation_requests","requested_plan","TEXT");
  await ensureColumn(env,"subscription_activation_requests","payment_phone","TEXT");
  await ensureColumn(env,"subscription_activation_requests","transaction_id","TEXT");
  await ensureColumn(env,"subscription_activation_requests","status","TEXT NOT NULL DEFAULT 'pending'");
  await ensureColumn(env,"subscription_activation_requests","handled_by","TEXT");
  ...
  if(v38Migrated?.value!=="1"){
    await env.DB.prepare(`INSERT OR IGNORE INTO trade_catalog(id,company_id,phase,name,created_at) SELECT lower(hex(randomblob(16))),company_id,COALESCE(NULLIF(trim(phase),''),'Autres'),trim(name),MIN(COALESCE(created_at,CURRENT_TIMESTAMP)) FROM trades WHERE trim(COALESCE(name,''))<>'' GROUP BY company_id,COALESCE(NULLIF(trim(phase),''),'Autres'),lower(trim(name))`).run();
    await env.DB.prepare(`UPDATE trades SET catalog_id=(SELECT tc.id FROM trade_catalog tc WHERE tc.company_id=trades.company_id AND lower(trim(tc.name))=lower(trim(trades.name)) AND lower(trim(tc.phase))=lower(trim(COALESCE(NULLIF(trades.phase,''),'Autres'))) LIMIT 1) WHERE catalog_id IS NULL OR catalog_id=''`).run();
    await env.DB.prepare(`UPDATE expenses SET trade_phase_snapshot=COALESCE(trade_phase_snapshot,(SELECT t.phase FROM trades t WHERE t.id=expenses.trade_id)),trade_name_snapshot=COALESCE(trade_name_snapshot,(SELECT t.name FROM trades t WHERE t.id=expenses.trade_id)) WHERE trade_id IS NOT NULL`).run();
...
async function load(req,env){
  ...
  if(s.u.role==="superadmin"){
    const [companies,users,resets,subscriptionRequests,closureRequests,logs]=await Promise.all([
      ...
      env.DB.prepare(`SELECT sr.*,c.name company_name,u.full_name requester_name,u.email requester_email
        FROM subscription_activation_requests sr
...
async function verifyAdminPassword(env,user,password){
  if(!user||user.role!=="admin"||!password)return false;
  const cr=await getMemberCredentialKV(env,user.id);if(!cr)return false;
  const h=await hashPassword(String(password),cr.password_salt,Number(cr.password_iterations||ITER));
  return await safeEq(h,cr.password_hash);
}
async function adminPasswordGate(env,s,password){
  if(s.u.role!=="admin")return json({error:"Action réservée à l'Administrateur"},403);
  if(!password)return json({error:"Mot de passe Administrateur requis"},403);
  if(!await verifyAdminPassword(env,s.u,password))return json({error:"Mot de passe Administrateur incorrect"},403);
  return null;
}
async function writableProject(env,companyId,projectId){
  const p=await env.DB.prepare("SELECT id,locked,status FROM projects WHERE id=? AND company_id=?").bind(projectId,companyId).first();
  if(!p)return {error:json({error:"Projet introuvable"},404)};
  if(Number(p.locked)===1)return {error:json({error:"Projet verrouillé. Déverrouillez-le avant toute opération."},423)};
  if(p.status==="closed")return {error:json({error:"Projet clôturé. Modifiez son statut avant toute opération."},423)};
  return {project:p};
}
...
async function saveCompany(req,env,s,entity,action,r){
  ...
  if(entity==="account"&&action==="update"){
    if(actor.role!=="admin")return json({error:"Modification réservée à l'Administrateur"},403);
    const companyName=text(r.company_name,180),adminName=text(r.admin_name,160),adminEmail=email(r.admin_email);
  ...
  if(entity==="subscription_request"&&action==="create"){
    if(actor.role!=="admin")return json({error:"Demande réservée à l'Administrateur"},403);
    const company=await env.DB.prepare("SELECT id,plan,plan_expires_at FROM companies WHERE id=? AND status='active'").bind(c).first();
    if(!company)return json({error:"Entreprise introuvable ou inactive"},404);
    const paidActive=["standard","business"].includes(String(company.plan||"").toLowerCase())&&Date.parse(company.plan_expires_at)>Date.now();
    if(paidActive)return json({error:"Votre abonnement payant est déjà actif. Aucune nouvelle demande ni paiement n'est autorisé pendant sa validité."},409);
    const pending=await env.DB.prepare("SELECT id FROM subscription_activation_requests WHERE company_id=? AND status='pending' LIMIT 1").bind(c).first();
    if(pending)return json({error:"Une demande d'activation est déjà en attente de traitement par le support."},409);
    const requestedPlan=normalizePlan(r.requested_plan);
    if(!["standard","business"].includes(requestedPlan))return json({error:"Choisissez la formule Standard ou Business"},400);
    const paymentPhone=text(r.payment_phone,50),transactionId=text(r.transaction_id,120);
    if(!paymentPhone||!transactionId)return json({error:"Numéro de paiement et ID de transaction obligatoires"},400);
    const duplicateTransaction=await env.DB.prepare("SELECT id FROM subscription_activation_requests WHERE lower(transaction_id)=lower(?) LIMIT 1").bind(transactionId).first();
    if(duplicateTransaction)return json({error:"Cet ID de transaction a déjà été transmis au support"},409);
    const id=crypto.randomUUID();
    await env.DB.prepare("INSERT INTO subscription_activation_requests(id,company_id,requested_by,requested_plan,payment_phone,transaction_id,status) VALUES(?,?,?,?,?,?,'pending')")
      .bind(id,c,actor.id,requestedPlan,paymentPhone,transactionId).run();
    await audit(env,actor,"SUBSCRIPTION_REQUEST","subscription_request",id,ip(req),{plan:requestedPlan});
    return json({ok:true,id,message:"Demande envoyée au support"});
  }
  ...
  if(entity==="project"){
    ...
    if(action==="update"){
      const denied=await adminPasswordGate(env,s,r.admin_password);if(denied)return denied;
      if(Number(own.locked)===1)return json({error:"Projet verrouillé. Déverrouillez-le d'abord."},423);
      await env.DB.prepare("UPDATE projects SET name=?,project_type=?,location=?,owner_name=?,manager_name=?,budget=?,start_date=?,end_date=?,description=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?")
        .bind(text(r.name,180),text(r.project_type,100),text(r.location,140),text(r.owner_name,160),text(r.manager_name,160),money(r.budget),r.start_date||null,r.end_date||null,text(r.description,1000),r.id,c).run();
      await audit(env,actor,"UPDATE_PROJECT","project",r.id,ip(req));return json({ok:true});
    }
    if(action==="set_status"){
      if(actor.role!=="admin")return json({error:"Action réservée à l'Administrateur"},403);
      const allowed=["preparation","in_progress","suspended","completed","closed"];
      const st=allowed.includes(r.status)?r.status:"in_progress";
    ...
    if(action==="delete"){
      ...
      await audit(env,actor,"DELETE_PROJECT","project",r.id,ip(req));return json({ok:true});
    }
  }
  if(entity==="trade_catalog"){
    const phase=text(r.phase,180),tradeName=text(r.name,120);
    if(!phase||!tradeName)return json({error:"Corps principal et activité obligatoires"},400);
    if(action==="create"){
      const dup=await env.DB.prepare("SELECT id FROM trade_catalog WHERE company_id=? AND lower(trim(phase))=lower(trim(?)) AND lower(trim(name))=lower(trim(?)) LIMIT 1").bind(c,phase,tradeName).first();
      if(dup)return json({error:"Ce métier existe déjà dans la liste générale",code:"CATALOG_TRADE_ALREADY_EXISTS"},409);
      const id=crypto.randomUUID();await env.DB.prepare("INSERT INTO trade_catalog(id,company_id,phase,name,created_by) VALUES(?,?,?,?,?)").bind(id,c,phase,tradeName,actor.id).run();await audit(env,actor,"CREATE_TRADE_CATALOG","trade_catalog",id,ip(req),{phase,name:tradeName});return json({ok:true,id});
    }
    if(action==="update"){
      const existing=await env.DB.prepare("SELECT id FROM trade_catalog WHERE id=? AND company_id=?").bind(r.id,c).first();if(!existing)return json({error:"Métier introuvable dans la liste générale"},404);
      const dup=await env.DB.prepare("SELECT id FROM trade_catalog WHERE company_id=? AND lower(trim(phase))=lower(trim(?)) AND lower(trim(name))=lower(trim(?)) AND id<>? LIMIT 1").bind(c,phase,tradeName,r.id).first();if(dup)return json({error:"Ce métier existe déjà dans la liste générale"},409);
      await env.DB.prepare("UPDATE trade_catalog SET phase=?,name=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?").bind(phase,tradeName,r.id,c).run();
      await env.DB.prepare("UPDATE trades SET phase=?,name=? WHERE catalog_id=? AND company_id=?").bind(phase,tradeName,r.id,c).run();
      await env.DB.prepare("UPDATE expenses SET trade_phase_snapshot=?,trade_name_snapshot=? WHERE company_id=? AND trade_id IN (SELECT id FROM trades WHERE company_id=? AND catalog_id=?)").bind(phase,tradeName,c,c,r.id).run();return json({ok:true});
    }
    if(action==="delete"&&actor.role==="admin"){
      const active=await env.DB.prepare(`SELECT p.name FROM trades t JOIN projects p ON p.id=t.project_id WHERE t.company_id=? AND t.catalog_id=? AND p.status!='closed' LIMIT 1`).bind(c,r.id).first();
      ...
      await env.DB.prepare("UPDATE trades SET catalog_id=NULL WHERE catalog_id=? AND company_id=?").bind(r.id,c).run();
      await env.DB.prepare("DELETE FROM trade_catalog WHERE id=? AND company_id=?").bind(r.id,c).run();await audit(env,actor,"DELETE_TRADE_CATALOG","trade_catalog",r.id,ip(req));return json({ok:true});
    }
  }
  if(entity==="trade"){
    if(action==="create"){
  ...
  if(entity==="expense"){
    if(action==="create"||action==="update"){
      ...
      const existing=await env.DB.prepare("SELECT id FROM expenses WHERE id=? AND company_id=? AND project_id=?").bind(r.id,c,projectId).first();if(!existing)return json({error:"Matériaux introuvables"},404);
      await env.DB.prepare("UPDATE expenses SET trade_id=?,supplier_id=?,expense_date=?,description=?,quantity=?,unit=?,unit_price=?,total_price=?,reference=?,trade_phase_snapshot=?,trade_name_snapshot=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND company_id=?").bind(trade.id,r.supplier_id||null,today(r.expense_date),text(r.description,500),qty(r.quantity),text(r.unit,40),money(r.unit_price),total,text(r.reference,120),trade.phase,trade.name,r.id,c).run();await audit(env,actor,"UPDATE_EXPENSE","expense",r.id,ip(req),{total});return json({ok:true,total});
    }
  ...
  if(entity==="labor"){
    if(action==="create"){
      ...
      const id=crypto.randomUUID();await insertLaborCompatible(env,{id,company_id:c,project_id:r.project_id,trade_id:r.trade_id||null,expense_date:today(r.expense_date),worker_name:text(r.worker_name,160),description:text(r.description,500),amount:money(r.amount),payment_method:text(r.payment_method,80),reference:text(r.reference,120),notes:text(r.notes,800),created_by:actor.id});await audit(env,actor,"CREATE_LABOR","labor",id,ip(req),{amount:money(r.amount)});return json({ok:true,id})
    }
...
async function saveSuper(req,env,s,entity,action,r){
  ...
  if(entity==="subscription_request"){
    const request=await env.DB.prepare("SELECT * FROM subscription_activation_requests WHERE id=?").bind(r.id).first();
    if(!request)return json({error:"Demande d'activation introuvable"},404);
    if(request.status!=="pending")return json({error:"Cette demande a déjà été traitée"},409);
    if(action==="approve"){
      const plan=normalizePlan(request.requested_plan);
      if(!["standard","business"].includes(plan))return json({error:"Formule demandée invalide"},400);
      const start=now(),end=plusDays(planDays(plan));
      await env.DB.prepare("UPDATE companies SET plan=?,plan_started_at=?,plan_expires_at=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(plan,start,end,request.company_id).run();
      await env.DB.prepare("UPDATE subscription_activation_requests SET status='approved',handled_by=?,support_note=?,handled_at=CURRENT_TIMESTAMP WHERE id=?").bind(actor.id,text(r.support_note,500),request.id).run();
      await audit(env,actor,"APPROVE_SUBSCRIPTION","subscription_request",request.id,ip(req),{company_id:request.company_id,plan});
      return json({ok:true});
    }
    if(action==="reject"){
      await env.DB.prepare("UPDATE subscription_activation_requests SET status='rejected',handled_by=?,support_note=?,handled_at=CURRENT_TIMESTAMP WHERE id=?").bind(actor.id,text(r.support_note,500),request.id).run();
      await audit(env,actor,"REJECT_SUBSCRIPTION","subscription_request",request.id,ip(req),{company_id:request.company_id});
      return json({ok:true});
    }
  }
  if(entity==="account_closure"){
