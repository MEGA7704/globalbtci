...
async function bootstrap(req,env){
  ...
  try{
    ...
    return json({ok:true,superadmin_ready:true,superadmin_auth:"cloudflare_secret",app_version:"33.0.0"});
  }catch(e){
    const msg=String(e?.message||"");
    console.error(JSON.stringify({event:"bootstrap_error",stage,message:msg,stack:e?.stack||""}));
    return json({error:"Initialisation Super Admin impossible",stage,code:msg.slice(0,120)||"BOOTSTRAP_ERROR",app_version:"33.0.0"},500);
  }
}
...
async function load(req,env){
  ...
  const [projects,trades,suppliers,projectSuppliers,expenses,labor,users,resets]=await Promise.all([
  ...
  return json({mode:"company",projects:projects.results,trades:trades.results,suppliers:suppliers.results,projectSuppliers:projectSuppliers.results,expenses:expenses.results,labor:labor.results,users:outUsers,resets:resets.results});
}
...
async function saveCompany(req,env,s,entity,action,r){
  ...
  if(entity==="expense"){
    ...
    if(action==="delete"&&actor.role==="admin"){const ex=await env.DB.prepare("SELECT id,project_id FROM expenses WHERE id=? AND company_id=?").bind(r.id,c).first();if(!ex)return json({error:"Dépense introuvable"},404);await env.DB.prepare("DELETE FROM expenses WHERE id=? AND company_id=?").bind(r.id,c).run();await audit(env,actor,"DELETE_EXPENSE","expense",r.id,ip(req),{project_id:ex.project_id});return json({ok:true})}
  }
  if(entity==="labor"){
    if(action==="create"){
