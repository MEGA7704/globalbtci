...
class CorporatePdf{
  constructor(orientation='portrait'){this.orientation=orientation==='landscape'?'landscape':'portrait';this.w=this.orientation==='landscape'?841.89:595.28;this.h=this.orientation==='landscape'?595.28:841.89;this.pages=[]}
  ...
  toBlob(){const objs=[];const add=b=>{objs.push(typeof b==='string'?pdfWinAnsiBytes(b):b);return objs.length};const font1=add('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');const font2=add('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>');const font3=add('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Oblique /Encoding /WinAnsiEncoding >>');const pagesObj=add('');const pageIds=[];for(const pg of this.pages){const content=pdfWinAnsiBytes(pg.ops.join(''));const contentObj=add(pdfConcat([pdfWinAnsiBytes(`<< /Length ${content.length} >>\nstream\n`),content,pdfWinAnsiBytes('\nendstream')]));const pageObj=add(`<< /Type /Page /Parent ${pagesObj} 0 R /MediaBox [0 0 ${pdfNum(this.w)} ${pdfNum(this.h)}] /Resources << /Font << /F1 ${font1} 0 R /F2 ${font2} 0 R /F3 ${font3} 0 R >> >> /Contents ${contentObj} 0 R >>`);pageIds.push(pageObj)}objs[pagesObj-1]=pdfWinAnsiBytes(`<< /Type /Pages /Count ${pageIds.length} /Kids [${pageIds.map(id=>`${id} 0 R`).join(' ')}] >>`);const catalog=add(`<< /Type /Catalog /Pages ${pagesObj} 0 R >>`);const header=pdfWinAnsiBytes('%PDF-1.4\n%\xE2\xE3\xCF\xD3\n');const parts=[header],offsets=[0];let offset=header.length;for(let i=0;i<objs.length;i++){offsets[i+1]=offset;const pre=pdfWinAnsiBytes(`${i+1} 0 obj\n`),post=pdfWinAnsiBytes('\nendobj\n');parts.push(pre,objs[i],post);offset+=pre.length+objs[i].length+post.length}const xref=offset;let trailer=`xref\n0 ${objs.length+1}\n0000000000 65535 f \n`;for(let i=1;i<=objs.length;i++)trailer+=`${String(offsets[i]).padStart(10,'0')} 00000 n \n`;trailer+=`trailer\n<< /Size ${objs.length+1} /Root ${catalog} 0 R >>\nstartxref\n${xref}\n%%EOF`;parts.push(pdfWinAnsiBytes(trailer));return new Blob(parts,{type:'application/pdf'})}
}
...
function printA4(title,subtitle,body,orientation='portrait',options={}){
  try{
    const c=S.session?.company||{},u=S.session?.user||{},pdf=new CorporatePdf(orientation);
    const W=pdf.w,H=pdf.h,land=orientation==='landscape',M=18;
    const company=c.name||'GLOBAL BT',slogan=c.slogan||'La qualité, notre engagement',phone=c.phone||u.phone||'—',mail=c.email||u.email||'—',address=[c.address,c.city].filter(Boolean).join(', ')||"Côte d’Ivoire",taxpayer=c.taxpayer_account||'—',rccm=c.rccm||'—',capital=Number(c.capital||0)>0?cash(c.capital):'—',editor=u.full_name||'Administrateur';
    ...
    const baseHeader=(p)=>{pdf.rect(p,0,0,W,H,{fill:[1,1,1]});const c1=land?W*.40:W*.43,c2=land?W*.70:W*.72;pdf.line(p,M,headerH,W-M,headerH,PDF_GREEN,1.4);pdf.line(p,c1,12,c1,headerH-12,[.15,.25,.23],.7);pdf.line(p,c2,12,c2,headerH-12,[.15,.25,.23],.7);drawBuilding(p,M+4,15,land?.95:.82);pdf.text(p,company,M+66,19,land?18:15,{font:'F2',color:PDF_GREEN,maxWidth:c1-M-74});pdf.text(p,'Gestion BTP Professionnelle',M+66,42,land?10:8.5,{font:'F2',color:PDF_GOLD,maxWidth:c1-M-74});pdf.text(p,slogan,M+66,59,land?8:7,{font:'F3',color:[.15,.27,.24],maxWidth:c1-M-74});pdf.line(p,M+66,73,Math.min(c1-18,M+175),73,PDF_GOLD,1);const mx=c1+18;pdf.text(p,'Adresse',mx,20,7,{font:'F2',color:PDF_GREEN});pdf.text(p,address,mx+42,20,8,{maxWidth:c2-mx-46});pdf.text(p,'Téléphone',mx,39,7,{font:'F2',color:PDF_GREEN});pdf.text(p,phone,mx+42,39,8,{maxWidth:c2-mx-46});pdf.text(p,'Email',mx,58,7,{font:'F2',color:PDF_GREEN});pdf.text(p,mail,mx+42,58,8,{maxWidth:c2-mx-46});const rx=c2+18;pdf.text(p,'N° Contribuable :',rx,20,7.5,{font:'F2',color:PDF_GREEN});pdf.text(p,taxpayer,rx+68,20,8,{maxWidth:W-M-rx-70});pdf.text(p,'RCCM :',rx,39,7.5,{font:'F2',color:PDF_GREEN});pdf.text(p,rccm,rx+68,39,8,{maxWidth:W-M-rx-70});pdf.text(p,'Capital :',rx,58,7.5,{font:'F2',color:PDF_GREEN});pdf.text(p,capital,rx+68,58,8,{maxWidth:W-M-rx-70});let y=headerH+14;pdf.line(p,M+30,y+14,W/2-95,y+14,PDF_GOLD,.9);pdf.line(p,W/2+95,y+14,W-M-30,y+14,PDF_GOLD,.9);pdf.text(p,String(title||'DOCUMENT').toUpperCase(),W/2,y,land?18:15,{font:'F2',color:PDF_GREEN,align:'center',maxWidth:180});if(subtitle)pdf.text(p,subtitle,W/2,y+23,6.8,{font:'F1',color:[.42,.48,.46],align:'center',maxWidth:260});y+=titleH;const metaY=y;pdf.rect(p,M,metaY,W-2*M,metaH,{fill:PDF_LIGHT,stroke:PDF_GRAY,lw:.7});const meta=[['Date d’édition',dateText],['Imprimé par',editor],['Portefeuille',portfolio],[options.totalLabel||'Document',options.total!==undefined?String(options.total):title]],mw=(W-2*M)/4;meta.forEach((it,i)=>{if(i)pdf.line(p,M+i*mw,metaY+8,M+i*mw,metaY+metaH-8,PDF_GRAY,.6);pdf.text(p,it[0]+' :',M+i*mw+10,metaY+10,6.4,{font:'F2',color:PDF_GREEN});pdf.text(p,it[1],M+i*mw+10,metaY+23,7.2,{maxWidth:mw-20})});y+=metaH+8;const sy=y,sw=(W-2*M)/3;pdf.rect(p,M,sy,W-2*M,sumH,{stroke:PDF_GOLD,lw:1});[[summaryOneLabel,summaryOne],[summaryTwoLabel,summaryTwo],[summaryThreeLabel,summaryThree]].forEach((it,i)=>{if(i)pdf.line(p,M+i*sw,sy+7,M+i*sw,sy+sumH-7,[.58,.61,.60],.6);pdf.text(p,it[0],M+i*sw+15,sy+10,6.4,{font:'F2',color:[.15,.26,.23]});pdf.text(p,it[1],M+i*sw+15,sy+24,land?9:8,{font:'F2',color:PDF_GREEN,maxWidth:sw-30})});return sy+sumH+10};
    ...
    const signatures=(p,y)=>{pdf.text(p,'Préparé par',M+10,y,8,{font:'F2',color:PDF_GREEN});pdf.line(p,M+10,y+13,M+62,y+13,PDF_GOLD,.7);['Nom :','Fonction :','Date :'].forEach((t,i)=>{pdf.text(p,t,M+10,y+24+i*18,7);pdf.line(p,M+48,y+31+i*18,M+150,y+31+i*18,[.3,.35,.34],.6)});const cx=W/2,cy=y+45;pdf.stroke(p,PDF_GREEN);{const r=28,k=r*.5522847498,py=H-cy;pdf.op(p,`${pdfNum(1)} w ${pdfNum(cx+r)} ${pdfNum(py)} m ${pdfNum(cx+r)} ${pdfNum(py+k)} ${pdfNum(cx+k)} ${pdfNum(py+r)} ${pdfNum(cx)} ${pdfNum(py+r)} c ${pdfNum(cx-k)} ${pdfNum(py+r)} ${pdfNum(cx-r)} ${pdfNum(py+k)} ${pdfNum(cx-r)} ${pdfNum(py)} c ${pdfNum(cx-r)} ${pdfNum(py-k)} ${pdfNum(cx-k)} ${pdfNum(py-r)} ${pdfNum(cx)} ${pdfNum(py-r)} c ${pdfNum(cx+k)} ${pdfNum(py-r)} ${pdfNum(cx+r)} ${pdfNum(py-k)} ${pdfNum(cx+r)} ${pdfNum(py)} c S\n`)}pdf.rect(p,cx-23,cy-23,46,46,{stroke:PDF_GREEN,lw:1});pdf.text(p,company,cx,cy-8,6.2,{font:'F2',color:PDF_GREEN,align:'center',maxWidth:40});pdf.text(p,'QUALITÉ',cx,cy+4,5.7,{font:'F2',color:PDF_GREEN,align:'center'});pdf.text(p,'ENGAGEMENT',cx,cy+14,5.3,{font:'F2',color:PDF_GREEN,align:'center'});pdf.text(p,'EXCELLENCE',cx,cy+24,5.3,{font:'F2',color:PDF_GREEN,align:'center'});const rx=W-M-160;pdf.text(p,'Approuvé par',rx,y,8,{font:'F2',color:PDF_GREEN});pdf.line(p,rx,y+13,rx+55,y+13,PDF_GOLD,.7);['Nom :','Fonction :','Date :'].forEach((t,i)=>{pdf.text(p,t,rx,y+24+i*18,7);pdf.line(p,rx+38,y+31+i*18,W-M-10,y+31+i*18,[.3,.35,.34],.6)})};
    const blocks=extractPdfBlocks(body);let p=pdf.page(),y=baseHeader(p),pageContentStart=y;
    const newPage=()=>{p=pdf.page();y=baseHeader(p);pageContentStart=y};
...
function reports(){const mat=S.data.expenses.reduce((a,x)=>a+Number(x.total_price),0),lab=S.data.labor.reduce((a,x)=>a+Number(x.amount),0);const by=S.data.trades.map(t=>({name:t.name,m:S.data.expenses.filter(x=>x.trade_id===t.id).reduce((a,x)=>a+Number(x.total_price),0),l:S.data.labor.filter(x=>x.trade_id===t.id).reduce((a,x)=>a+Number(x.amount),0)}));$("#content").innerHTML=`<div class="kpis">${kpi("Matériaux",cash(mat))}${kpi("Main-d'œuvre",cash(lab))}${kpi("Total",cash(mat+lab))}</div><div class="panel"><div class="panelhead"><h2>Bilan par métier</h2><button onclick="printA4('Bilan par métier','Rapport de gestion',this.closest('.panel').querySelector('.tablewrap')?.outerHTML||'', 'portrait')" class="btn secondary">PDF A4</button></div>${table(["Métier","Matériaux","Main-d'œuvre","Total"],by.map(x=>`<tr><td>${esc(x.name)}</td><td class="money">${cash(x.m)}</td><td class="money">${cash(x.l)}</td><td class="money"><strong>${cash(x.m+x.l)}</strong></td></tr>`))}</div>`}
