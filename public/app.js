...
function projects(){
  const rows=S.data.projects.map(x=>`<tr><td><strong>${esc(x.name)}</strong><br><small>${esc(x.location||'')}</small></td><td><span class="status">${esc({preparation:'Préparation',in_progress:'En cours',suspended:'Suspendu',completed:'Terminé',closed:'Clôturé'}[x.status]||x.status)}</span></td><td class="money">${cash(x.budget)}</td><td>${Number(x.locked)?'<span class="status disabled">Verrouillé</span>':'<span class="status">Déverrouillé</span>'}</td><td><div class="project-actions"><button class="btn small secondary v27Edit" data-id="${x.id}">Modifier</button><button class="btn small secondary v27Trades" data-id="${x.id}">Métiers</button><button class="btn small secondary v27Expenses" data-id="${x.id}">Dépenses</button><button class="btn small secondary v27Suppliers" data-id="${x.id}">Fournisseurs</button>${S.session.user.role==='admin'?`<button class="btn small secondary v27Lock" data-id="${x.id}" data-act="${Number(x.locked)?'unlock':'lock'}">${Number(x.locked)?'Déverrouiller':'Verrouiller'}</button><button class="btn small secondary v27Status" data-id="${x.id}">Statut</button><button class="btn small danger v27Delete" data-id="${x.id}">Supprimer</button>`:''}</div></td></tr>`);
  $('#content').innerHTML=`<div class="panel"><div class="panelhead"><div><h2>Projets</h2><p class="muted">Chaque projet regroupe ses métiers, dépenses, fournisseurs et main-d'œuvre.</p></div>${S.session.user.role==='admin'?'<button id="v27NewProject" class="btn primary">+ Nouveau projet</button>':''}</div>${table(['Projet','Statut','Budget','Sécurité','Actions'],rows)}</div>`;
  $('#v27NewProject')?.addEventListener('click',()=>modal(`<h2>Nouveau projet</h2><form id="v27ProjectCreate" class="formgrid"><label>Nom<input name="name" required></label><label>Type<input name="project_type" value="Bâtiment"></label><label>Localité<input name="location"></label><label>Budget<input name="budget" type="number" min="0"></label><label>Maître d'ouvrage<input name="owner_name"></label><label>Responsable<input name="manager_name"></label><label>Date début<input name="start_date" type="date"></label><label>Date fin<input name="end_date" type="date"></label><label class="span2">Description<textarea name="description"></textarea></label><button class="btn primary span2">Créer</button></form>`));
  document.querySelectorAll('.v27Edit').forEach(b=>b.onclick=()=>{const x=S.data.projects.find(p=>p.id===b.dataset.id);if(S.session.user.role!=='admin')return toast('Modification réservée à l’Administrateur',true);modal(`<h2>Modifier le projet</h2><form id="v27ProjectEdit" data-id="${x.id}" class="formgrid"><label>Nom<input name="name" value="${esc(x.name)}" required></label><label>Type<input name="project_type" value="${esc(x.project_type||'')}"></label><label>Localité<input name="location" value="${esc(x.location||'')}"></label><label>Budget<input name="budget" type="number" value="${Number(x.budget||0)}"></label><label>Maître d'ouvrage<input name="owner_name" value="${esc(x.owner_name||'')}"></label><label>Responsable<input name="manager_name" value="${esc(x.manager_name||'')}"></label><label>Date début<input name="start_date" type="date" value="${esc(x.start_date||'')}"></label><label>Date fin<input name="end_date" type="date" value="${esc(x.end_date||'')}"></label><label class="span2">Description<textarea name="description">${esc(x.description||'')}</textarea></label><label class="span2">Mot de passe Administrateur<input name="admin_password" type="password" required></label><button class="btn primary span2">Enregistrer</button></form>`)});
  document.querySelectorAll('.v27Trades').forEach(b=>b.onclick=()=>v27ProjectTrades(b.dataset.id));document.querySelectorAll('.v27Expenses').forEach(b=>b.onclick=()=>v27ProjectExpenses(b.dataset.id));document.querySelectorAll('.v27Suppliers').forEach(b=>b.onclick=()=>v27ProjectSuppliers(b.dataset.id));
  document.querySelectorAll('.v27Lock').forEach(b=>b.onclick=()=>adminGate(b.dataset.act==='lock'?'Verrouiller le projet':'Déverrouiller le projet',async pw=>{await post('/api/save',{entity:'project',action:b.dataset.act,record:{id:b.dataset.id,admin_password:pw}});closeModal();await reload();projects();toast('Projet mis à jour')}));
  document.querySelectorAll('.v27Status').forEach(b=>b.onclick=()=>{const x=S.data.projects.find(p=>p.id===b.dataset.id);modal(`<h2>Statut</h2><form id="v27StatusForm" data-id="${x.id}"><label>Statut<select name="status"><option value="preparation">Préparation</option><option value="in_progress">En cours</option><option value="suspended">Suspendu</option><option value="completed">Terminé</option><option value="closed">Clôturé</option></select></label><button class="btn primary full">Mettre à jour</button></form>`);$('#v27StatusForm').status.value=x.status});
  document.querySelectorAll('.v27Delete').forEach(b=>b.onclick=()=>adminGate('Supprimer le projet',async pw=>{await post('/api/save',{entity:'project',action:'delete',record:{id:b.dataset.id,admin_password:pw}});closeModal();await reload();projects();toast('Projet supprimé')}));
}
