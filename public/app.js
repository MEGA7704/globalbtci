...
function v33ProjectHub(projectId,initialView="trades"){
  ...
  const show=view=>{
    ...
    if(view==="trades"){
      ...
      document.querySelectorAll(".ppm-delete-trade").forEach(b=>b.onclick=()=>confirmBox("Supprimer ce métier ?",async()=>{await post("/api/save",{entity:"trade",action:"delete",record:{id:b.dataset.id}});await reload();closeModal();v33ProjectHub(projectId,"trades");toast("Métier supprimé")}));
    ...
    if(view==="expenses"){
      ...
      document.querySelectorAll(".ppm-delete-expense").forEach(b=>b.onclick=()=>confirmBox("Supprimer cette dépense du projet ?",async()=>{await post("/api/save",{entity:"expense",action:"delete",record:{id:b.dataset.id}});await reload();closeModal();v33ProjectHub(projectId,"expenses");toast("Dépense supprimée")}));
      bindPager(view,pg.pages);
    }
    if(view==="suppliers"){
      ...
      document.querySelectorAll(".ppm-delete-supplier").forEach(b=>b.onclick=()=>confirmBox("Retirer ce fournisseur du projet ?",async()=>{await post("/api/save",{entity:"project_supplier",action:"delete",record:{id:b.dataset.id,project_id:projectId}});await reload();closeModal();v33ProjectHub(projectId,"suppliers");toast("Fournisseur retiré du projet")}));
      bindPager(view,pg.pages);
    }
  };
...
function projects(){
  ...
  document.querySelectorAll('.v28-project-row').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;v33ProjectHub(r.dataset.id)});
...
document.addEventListener('submit',async e=>{const f=e.target;if(!f.id?.startsWith('v28'))return;e.preventDefault();try{if(f.id==='v28ProjectCreate'){await post('/api/save',{entity:'project',action:'create',record:fd(f)});closeModal();await reload();projects();toast('Projet créé')}else if(f.id==='v28ProjectEdit'){await post('/api/save',{entity:'project',action:'update',record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();projects();toast('Projet modifié')}else if(f.id==='v28StatusForm'){await post('/api/save',{entity:'project',action:'set_status',record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();projects();toast('Statut mis à jour')}else if(f.id==='v28TradeForm'){await post('/api/save',{entity:'trade',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v33ProjectHub(f.dataset.project);toast('Métier ajouté')}else if(f.id==='v28ExpenseForm'){await post('/api/save',{entity:'expense',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v33ProjectHub(f.dataset.project);toast('Dépense enregistrée')}else if(f.id==='v28SupplierForm'){await post('/api/save',{entity:'project_supplier',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v33ProjectHub(f.dataset.project);toast('Fournisseur affecté')}}catch(x){toast(x.message,true)}finally{releaseForm(f)}},true);
...
document.addEventListener("submit",async e=>{
  ...
  try{
    if(f.id==="v29TradeForm"){
      ...
      await reload();closeModal();v33ProjectHub(f.dataset.project);toast("Métier ajouté");
      ...
      await reload();closeModal();v33ProjectHub(f.dataset.project);toast("Métier modifié");
    }else if(f.id==="v29ExpenseForm"){
      await post("/api/save",{entity:"expense",action:"create",record:{project_id:f.dataset.project,...fd(f)}});
      await reload();closeModal();v33ProjectHub(f.dataset.project);toast("Dépense enregistrée");
      ...
      await reload();closeModal();v33ProjectHub(f.dataset.project);toast("Fournisseur affecté");
    }
