...
function v28ProjectHub(projectId){
  ...
  const laborTotal=(S.data.labor||[]).filter(x=>x.project_id===projectId).reduce((a,x)=>a+Number(x.amount||0),0);
  const expenseTotal=expenses.reduce((a,x)=>a+Number(x.total_price||0),0);
  modal(`<div class="project-hub"><div class="project-hub-head"><div><span class="eyebrow">ESPACE PROJET</span><h2>${esc(p.name)}</h2><p>${esc(p.location||"")}</p></div><div class="project-hub-kpis"><span><small>Budget</small><strong>${cash(p.budget)}</strong></span><span><small>Dépenses</small><strong>${cash(expenseTotal)}</strong></span><span><small>Main-d'œuvre</small><strong>${cash(laborTotal)}</strong></span></div></div><div class="project-hub-menu"><button class="project-hub-tab active" data-view="trades">Métiers <b>${trades.length}</b></button><button class="project-hub-tab" data-view="expenses">Dépenses <b>${expenses.length}</b></button><button class="project-hub-tab" data-view="suppliers">Fournisseurs <b>${suppliers.length}</b></button></div><div id="projectHubContent" class="project-hub-content"></div></div>`);
  const content=$("#projectHubContent"),tabs=[...document.querySelectorAll('.project-hub-tab')];
  const show=view=>{tabs.forEach(b=>b.classList.toggle('active',b.dataset.view===view));
    if(view==='trades'){content.innerHTML=`<div class="project-hub-toolbar"><div><h3>Métiers du projet</h3><p>Ajouter ou consulter les métiers affectés.</p></div><button id="hubAddTrade" class="btn primary">+ Ajouter un métier</button></div>${table(['Métier','Phase','Main-d\'œuvre'],trades.map(t=>{const mt=(S.data.labor||[]).filter(l=>l.project_id===projectId&&l.trade_id===t.id).reduce((a,x)=>a+Number(x.amount||0),0);return `<tr><td><strong>${esc(t.name)}</strong></td><td>${esc(phaseShort(t.phase||phaseForTrade(t.name)))}</td><td class="money">${cash(mt)}</td></tr>`}))}`;$("#hubAddTrade").onclick=()=>modal(`<h2>Ajouter un métier</h2><form id="v28TradeForm" data-project="${projectId}"><label>Phase<select name="phase">${TRADE_PHASES.map(ph=>`<option value="${esc(ph.key)}">${esc(ph.label)}</option>`).join('')}</select></label><label>Métier<input name="name" required></label><label>Description<textarea name="description"></textarea></label><button class="btn primary full">Ajouter</button></form>`)}
    else if(view==='expenses'){content.innerHTML=`<div class="project-hub-toolbar"><div><h3>Dépenses du projet</h3><p>Dépenses classées par métier.</p></div><button id="hubAddExpense" class="btn primary">+ Ajouter une dépense</button></div>${table(['Date','Métier','Désignation','Fournisseur','Montant'],expenses.map(x=>`<tr><td>${df(x.expense_date)}</td><td>${esc(x.trade_name||'—')}</td><td>${esc(x.description||'')}</td><td>${esc(x.supplier_name||'—')}</td><td class="money">${cash(x.total_price)}</td></tr>`))}`;$("#hubAddExpense").onclick=()=>modal(`<h2>Nouvelle dépense</h2><form id="v28ExpenseForm" data-project="${projectId}"><label>Métier<select name="trade_id">${opts(trades)}</select></label><label>Fournisseur<select name="supplier_id">${opts(S.data.suppliers||[])}</select></label><label>Date<input name="expense_date" type="date" value="${new Date().toISOString().slice(0,10)}"></label><label>Désignation<input name="description" required></label><label>Quantité<input name="quantity" type="number" step=".01" value="1"></label><label>Unité<input name="unit"></label><label>Prix unitaire<input name="unit_price" type="number" min="0"></label><label>Référence<input name="reference"></label><button class="btn primary full">Enregistrer</button></form>`)}
    else{content.innerHTML=`<div class="project-hub-toolbar"><div><h3>Fournisseurs du projet</h3><p>Affecter les fournisseurs intervenant sur ce chantier.</p></div><button id="hubAddSupplier" class="btn primary">+ Affecter un fournisseur</button></div>${table(['Fournisseur','Contact','Spécialité','Dépenses'],suppliers.map(x=>{const spent=expenses.filter(e=>e.supplier_id===x.supplier_id).reduce((a,e)=>a+Number(e.total_price||0),0);return `<tr><td><strong>${esc(x.supplier_name)}</strong></td><td>${esc(x.phone||'')}</td><td>${esc(x.specialty||'')}</td><td class="money">${cash(spent)}</td></tr>`}))}`;$("#hubAddSupplier").onclick=()=>modal(`<h2>Affecter un fournisseur</h2><form id="v28SupplierForm" data-project="${projectId}"><label>Fournisseur<select name="supplier_id" required>${opts(S.data.suppliers||[])}</select></label><label>Notes<textarea name="notes"></textarea></label><button class="btn primary full">Affecter</button></form>`)}
  };tabs.forEach(b=>b.onclick=()=>show(b.dataset.view));show('trades');
}
...
function projects(){
  ...
  document.querySelectorAll('.v28-project-row').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;v28ProjectHub(r.dataset.id)});
