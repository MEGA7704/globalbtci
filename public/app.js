...
function printA4(title,subtitle,body,orientation="portrait",options={}){
  const w=window.open("","_blank","width=1280,height=900");if(!w){toast("Autorisez les popups pour imprimer.",true);return}
  ...
  const legal=[c.taxpayer_account?`Compte contribuable : ${c.taxpayer_account}`:"",c.rccm?`RCCM : ${c.rccm}`:"",Number(c.capital||0)>0?`Capital : ${cash(c.capital)}`:""].filter(Boolean);
  const stamp=new Date().toLocaleString("fr-FR"),editor=u.full_name||"Administrateur";
  const info=Array.isArray(options.info)?options.info:[];
  const totalText=options.totalLabel&&options.total!==undefined?`${options.totalLabel} : ${options.total}`:"";
  const titleSub=subtitle||"Document officiel de gestion BTP";
  const infoHtml=info.length?`<div class="doc-info">${info.map(x=>`<div><small>${esc(x.label||"")}</small><strong>${esc(x.value??"—")}</strong></div>`).join("")}</div>`:"";
  const building=`<svg viewBox="0 0 64 64" aria-hidden="true"><path d="M10 54h44M15 54V24l17-10 17 10v30M23 54V32h18v22M27 37h4v5h-4zm6 0h4v5h-4zm-6 9h4v5h-4zm6 0h4v5h-4z"/></svg>`;
  w.document.write(`<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>${esc(title)}</title><style>
  @page{size:A4 ${orientation==="landscape"?"landscape":"portrait"};margin:9mm 10mm 12mm}
  *{box-sizing:border-box}html,body{margin:0;padding:0}body{font-family:"Segoe UI",Arial,Helvetica,sans-serif;color:#17302c;background:#fff;font-size:9.3px;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  .sheet{min-height:${orientation==="landscape"?"188mm":"275mm"};display:flex;flex-direction:column}
  .company-head{display:grid;grid-template-columns:68px 1fr 1.05fr;gap:14px;align-items:center;padding:10px 12px;background:linear-gradient(110deg,#f7faf9,#fff);border:1px solid #d9e3e0;border-top:5px solid #064f47;border-radius:12px 12px 4px 4px}
  .logo{width:62px;height:62px;border-radius:14px;background:#064f47;display:grid;place-items:center;box-shadow:0 5px 14px rgba(6,79,71,.15)}.logo svg{width:43px;height:43px;fill:none;stroke:#d9ad45;stroke-width:3;stroke-linecap:round;stroke-linejoin:round}
  .company-name{margin:0;color:#063e39;font-size:21px;line-height:1.05;letter-spacing:.015em}.company-sub{margin:5px 0 0;font-weight:800;color:#8a6720;font-size:9px;text-transform:uppercase;letter-spacing:.08em}.company-slogan{margin:5px 0 0;color:#60716d;font-style:italic}
  .contact{border-left:1px solid #d7e0de;padding-left:14px;color:#536762;line-height:1.6}.contact b{color:#173f39}.legal{margin-top:4px;font-size:8px;color:#72817e}
  .doc-title{text-align:center;padding:15px 8px 9px}.doc-title h1{margin:0;color:#063f39;font-size:19px;letter-spacing:.045em;text-transform:uppercase}.doc-title p{margin:5px 0;color:#687975}.gold-line{width:72px;height:3px;background:#d6a83e;border-radius:5px;margin:8px auto 0}
  .doc-info{display:grid;grid-template-columns:repeat(${orientation==="landscape"?"4":"2"},1fr);gap:7px;margin:4px 0 12px;padding:9px;background:#f5f8f7;border:1px solid #dce5e2;border-radius:9px}.doc-info div{padding:2px 5px;border-left:3px solid #d6a83e}.doc-info small{display:block;color:#72817e;text-transform:uppercase;font-size:7px;letter-spacing:.06em;margin-bottom:3px}.doc-info strong{display:block;color:#143d37;font-size:9px}
  .content{flex:1}.content h2,.content h3{color:#064f47}.content h3{font-size:11px;margin:12px 0 6px}
  table{width:100%;border-collapse:separate;border-spacing:0;font-size:${orientation==="landscape"?"8.2":"8.6"}px;border:1px solid #cbd8d5;border-radius:7px;overflow:hidden}th,td{padding:6px 6px;vertical-align:middle;text-align:left;border-right:1px solid #d6e0de;border-bottom:1px solid #d6e0de}th:last-child,td:last-child{border-right:0}tbody tr:last-child td{border-bottom:0}th{background:#064f47;color:#fff;font-weight:800;text-transform:uppercase;font-size:7.4px;letter-spacing:.025em}tbody tr:nth-child(even){background:#f6f8f8}tr{break-inside:avoid}.money{text-align:right!important}.center{text-align:center!important}.nowrap{white-space:nowrap}
  .print-spacer{min-height:18mm}.footer-meta{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-top:12px;padding:7px 2px;color:#60716d;border-top:1px solid #d8e1df;font-size:8px}.footer-meta strong{color:#174b43}
  .footer-band{display:grid;grid-template-columns:1fr auto;gap:14px;align-items:center;background:#064f47;color:#fff;padding:8px 12px;border-radius:5px}.footer-band b{color:#f2cd70}.footer-band small{display:block;color:#d8e8e4;margin-top:2px}.quality{font-weight:800;color:#f2cd70;white-space:nowrap}
  </style></head><body><main class="sheet">
  <header class="company-head"><div class="logo">${building}</div><div><h2 class="company-name">${esc(company)}</h2><p class="company-sub">Gestion BTP Professionnelle</p><p class="company-slogan">${esc(slogan)}</p></div><div class="contact"><div><b>Téléphone :</b> ${esc(phone)}</div><div><b>Email :</b> ${esc(mail)}</div><div><b>Adresse :</b> ${esc(address)}</div>${legal.length?`<div class="legal">${legal.map(esc).join(" · ")}</div>`:""}</div></header>
  <section class="doc-title"><h1>${esc(title)}</h1><p>${esc(titleSub)}</p><div class="gold-line"></div></section>
  ${infoHtml}<section class="content">${body}<div class="print-spacer"></div></section>
  <div class="footer-meta"><strong>${esc(totalText)}</strong><span>Édité le ${esc(stamp)} · Édité par : ${esc(editor)} · Page 1 / 1</span></div>
  <footer class="footer-band"><div><b>${esc(company)}</b><small>${esc(slogan)}</small></div><div class="quality">La qualité, notre engagement</div></footer>
...
function projects(){
  ...
  $("#v41PrintProjects").onclick=()=>{const list=visible();const body=`<table><tr><th>N° projet</th><th>Nom du projet</th><th>Type</th><th>Localité</th><th>Maître d'ouvrage</th><th>Responsable</th><th>Date début / Date fin</th><th>Statut</th></tr>${list.map(x=>`<tr><td class="nowrap"><strong>${esc(x.project_number||"—")}</strong></td><td>${esc(x.name)}</td><td>${esc(x.project_type||"—")}</td><td>${esc(x.location||"—")}</td><td>${esc(x.owner_name||"—")}</td><td>${esc(x.manager_name||"—")}</td><td class="nowrap">${df(x.start_date)||"—"} / ${df(x.end_date)||"—"}</td><td>${esc(statusLabel(x.status))}</td></tr>`).join("")}</table>`;printA4("Liste des projets",query?`Filtre : ${query}`:"Tous les projets",body,"landscape",{info:[{label:"Document",value:"Registre des projets"},{label:"Recherche",value:query||"Aucun filtre"},{label:"Date d’édition",value:new Date().toLocaleDateString("fr-FR")},{label:"Édité par",value:S.session.user.full_name||"Administrateur"}],totalLabel:"Total projets",total:list.length})};
