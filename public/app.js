...
function v29ProjectHub(projectId){
  ...
  modal(`
    <div class="project-premium-modal">
      <div class="ppm-top">
      ...
      <div class="ppm-body">
        <div class="ppm-tabs">
          <button class="ppm-tab active" data-view="trades"><span>▣</span> Métiers <b>${trades.length}</b></button>
          <button class="ppm-tab" data-view="expenses"><span>▧</span> Dépenses <b>${expenses.length}</b></button>
          <button class="ppm-tab" data-view="suppliers"><span>♟</span> Fournisseurs <b>${suppliers.length}</b></button>
        </div>
        ...
        <div class="ppm-footer">
          <div class="ppm-info">
            <span class="ppm-info-icon">ⓘ</span>
            <div><strong id="ppmFooterCount">${trades.length} métiers au total</strong><small>Mis à jour le ${esc(updated)}</small></div>
          </div>
  ...
  const content=$("#ppmContent"),tabs=[...document.querySelectorAll(".ppm-tab")];
  const footerCount=$("#ppmFooterCount");

  const tradeActions=t=>`
    <div class="ppm-row-actions">
      <button type="button" class="ppm-icon-btn ppm-view-trade" data-id="${t.id}" title="Voir">◉</button>
      <button type="button" class="ppm-icon-btn ppm-edit-trade" data-id="${t.id}" title="Modifier">✎</button>
      ${S.session.user.role==="admin"?`<button type="button" class="ppm-icon-btn danger ppm-delete-trade" data-id="${t.id}" title="Supprimer">♜</button>`:""}
    </div>`;
  ...
  const show=view=>{
    ...
    if(view==="trades"){
      ...
      content.innerHTML=`
        <div class="ppm-section-head">
          <div><h3>Métiers du projet</h3><p>Consultez et gérez les métiers affectés à ce projet.</p></div>
          <button id="ppmAddTrade" class="btn ppm-primary">+ Ajouter un métier</button>
        </div>
          ...
          <table class="ppm-table">
            ...
            <tbody>
              ${trades.length?trades.map(t=>{
                ...
                return `<tr>
                  <td><div class="ppm-trade-cell"><span class="ppm-trade-icon">${v29TradeIcon(t.name)}</span><strong>${esc(t.name)}</strong></div></td>
                  <td><span class="ppm-phase ${phase==="01_acquisition_etudes"?"blue":""}">${esc(phaseShort(phase))}</span></td>
                  <td class="ppm-money">${cash(mt)}</td>
                  <td>${tradeActions(t)}</td>
                </tr>`;
              }).join(""):`<tr><td colspan="4" class="empty">Aucun métier affecté à ce projet</td></tr>`}
            </tbody>
          </table>
        </div>`;

      $("#ppmAddTrade").onclick=()=>modal(`
        <h2>Ajouter un métier · ${esc(p.name)}</h2>
        <form id="v29TradeForm" data-project="${projectId}">
          <label>Phase<select name="phase">${TRADE_PHASES.map(ph=>`<option value="${esc(ph.key)}">${esc(ph.label)}</option>`).join("")}</select></label>
          <label>Métier<input name="name" required></label>
          <label>Description<textarea name="description"></textarea></label>
          <button class="btn primary full">Ajouter</button>
        </form>`);
      ...
      document.querySelectorAll(".ppm-view-trade").forEach(b=>b.onclick=()=>{
        ...
        modal(`<h2>${esc(t.name)}</h2><p><strong>Phase :</strong> ${esc(phaseLabel(t.phase||phaseForTrade(t.name)))}</p>
          <p><strong>Description :</strong> ${esc(t.description||"—")}</p>
          <h3>Main-d'œuvre</h3>
          ${table(["Date","Prestataire","Travaux","Montant"],labor.map(x=>`<tr><td>${df(x.expense_date)}</td><td>${esc(x.worker_name||"")}</td><td>${esc(x.description||"")}</td><td class="money">${cash(x.amount)}</td></tr>`))}`);
      });

      document.querySelectorAll(".ppm-edit-trade").forEach(b=>b.onclick=()=>{
        const t=trades.find(x=>x.id===b.dataset.id);if(!t)return;
        modal(`<h2>Modifier le métier</h2><form id="v29TradeEditForm" data-id="${t.id}" data-project="${projectId}">
          ...
          <button class="btn primary full">Enregistrer</button></form>`);
      });

      document.querySelectorAll(".ppm-delete-trade").forEach(b=>b.onclick=()=>confirmBox("Supprimer ce métier ?",async()=>{
        await post("/api/save",{entity:"trade",action:"delete",record:{id:b.dataset.id}});
        await reload();closeModal();v29ProjectHub(projectId);toast("Métier supprimé");
      }));
    }
    ...
    if(view==="expenses"){
      ...
      content.innerHTML=`
        <div class="ppm-section-head">
          <div><h3>Dépenses du projet</h3><p>Consultez les dépenses affectées aux différents métiers.</p></div>
          <button id="ppmAddExpense" class="btn ppm-primary">+ Ajouter une dépense</button>
        </div>
        <div class="ppm-table-wrap"><table class="ppm-table">
          <thead><tr><th>DATE</th><th>MÉTIER</th><th>DÉSIGNATION</th><th>FOURNISSEUR</th><th>MONTANT</th></tr></thead>
          <tbody>${expenses.length?expenses.map(x=>`<tr><td>${df(x.expense_date)}</td><td>${esc(x.trade_name||"—")}</td><td><strong>${esc(x.description||"")}</strong></td><td>${esc(x.supplier_name||"—")}</td><td class="ppm-money">${cash(x.total_price)}</td></tr>`).join(""):`<tr><td colspan="5" class="empty">Aucune dépense enregistrée</td></tr>`}</tbody>
        </table></div>`;
      $("#ppmAddExpense").onclick=()=>modal(`<h2>Nouvelle dépense · ${esc(p.name)}</h2><form id="v29ExpenseForm" data-project="${projectId}">
    ...
    if(view==="suppliers"){
      ...
      content.innerHTML=`
        <div class="ppm-section-head">
          <div><h3>Fournisseurs du projet</h3><p>Consultez et affectez les fournisseurs intervenant sur ce projet.</p></div>
          <button id="ppmAddSupplier" class="btn ppm-primary">+ Affecter un fournisseur</button>
        </div>
        <div class="ppm-table-wrap"><table class="ppm-table">
          <thead><tr><th>FOURNISSEUR</th><th>CONTACT</th><th>SPÉCIALITÉ</th><th>DÉPENSES</th></tr></thead>
          <tbody>${suppliers.length?suppliers.map(x=>{
            const spent=expenses.filter(e=>e.supplier_id===x.supplier_id).reduce((a,e)=>a+Number(e.total_price||0),0);
            return `<tr><td><strong>${esc(x.supplier_name)}</strong></td><td>${esc(x.phone||"—")}</td><td>${esc(x.specialty||"—")}</td><td class="ppm-money">${cash(spent)}</td></tr>`;
          }).join(""):`<tr><td colspan="4" class="empty">Aucun fournisseur affecté</td></tr>`}</tbody>
        </table></div>`;
      $("#ppmAddSupplier").onclick=()=>modal(`<h2>Affecter un fournisseur · ${esc(p.name)}</h2><form id="v29SupplierForm" data-project="${projectId}">
        <label>Fournisseur<select name="supplier_id" required>${opts(S.data.suppliers||[])}</select></label>
        <label>Notes<textarea name="notes"></textarea></label>
        <button class="btn primary full">Affecter</button></form>`);
    }
  };
...
function projects(){
  ...
  document.querySelectorAll('.v28-project-row').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;v29ProjectHub(r.dataset.id)});
...
document.addEventListener('submit',async e=>{const f=e.target;if(!f.id?.startsWith('v28'))return;e.preventDefault();try{if(f.id==='v28ProjectCreate'){await post('/api/save',{entity:'project',action:'create',record:fd(f)});closeModal();await reload();projects();toast('Projet créé')}else if(f.id==='v28ProjectEdit'){await post('/api/save',{entity:'project',action:'update',record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();projects();toast('Projet modifié')}else if(f.id==='v28StatusForm'){await post('/api/save',{entity:'project',action:'set_status',record:{id:f.dataset.id,...fd(f)}});closeModal();await reload();projects();toast('Statut mis à jour')}else if(f.id==='v28TradeForm'){await post('/api/save',{entity:'trade',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v29ProjectHub(f.dataset.project);toast('Métier ajouté')}else if(f.id==='v28ExpenseForm'){await post('/api/save',{entity:'expense',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v29ProjectHub(f.dataset.project);toast('Dépense enregistrée')}else if(f.id==='v28SupplierForm'){await post('/api/save',{entity:'project_supplier',action:'create',record:{project_id:f.dataset.project,...fd(f)}});await reload();closeModal();v29ProjectHub(f.dataset.project);toast('Fournisseur affecté')}}catch(x){toast(x.message,true)}finally{releaseForm(f)}},true);
...
document.addEventListener("submit",async e=>{
  ...
  try{
    if(f.id==="v29TradeForm"){
      ...
      await reload();closeModal();v29ProjectHub(f.dataset.project);toast("Métier ajouté");
      ...
      await reload();closeModal();v29ProjectHub(f.dataset.project);toast("Métier modifié");
    }else if(f.id==="v29ExpenseForm"){
      await post("/api/save",{entity:"expense",action:"create",record:{project_id:f.dataset.project,...fd(f)}});
      await reload();closeModal();v29ProjectHub(f.dataset.project);toast("Dépense enregistrée");
      ...
      await reload();closeModal();v29ProjectHub(f.dataset.project);toast("Fournisseur affecté");
    }
