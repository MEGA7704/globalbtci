...
function v32ProjectHub(projectId,initialView="trades"){
  ...
  const pageSize=()=>window.innerHeight<760?3:window.innerHeight<900?4:5;
  const page={trades:1,expenses:1,suppliers:1};

  modal(`<div class="project-premium-modal">
    <div class="ppm-top"><div class="ppm-label"><span class="ppm-label-icon">▣</span> ESPACE PROJET</div><button type="button" class="ppm-close" onclick="closeModal()" aria-label="Fermer">×</button></div>
    <div class="ppm-hero"><div class="ppm-project"><div class="ppm-building">🏢</div><div><h2>${esc(p.name)}</h2><p>⌖ ${esc(p.location||"Localité non renseignée")}</p></div></div>
      <div class="ppm-kpis"><div class="ppm-kpi ppm-budget"><span class="ppm-kpi-icon">▣</span><div><small>Budget</small><strong>${cash(p.budget)}</strong></div></div><div class="ppm-kpi ppm-expense"><span class="ppm-kpi-icon">↗</span><div><small>Dépenses</small><strong>${cash(expenseTotal)}</strong></div></div><div class="ppm-kpi ppm-labor"><span class="ppm-kpi-icon">♟</span><div><small>Main-d'œuvre</small><strong>${cash(laborTotal)}</strong></div></div></div>
    </div>
    <div class="ppm-body"><div class="ppm-tabs"><button class="ppm-tab" data-view="trades"><span>▣</span> Métiers <b>${trades.length}</b></button><button class="ppm-tab" data-view="expenses"><span>▧</span> Dépenses <b>${expenses.length}</b></button><button class="ppm-tab" data-view="suppliers"><span>♟</span> Fournisseurs <b>${suppliers.length}</b></button></div>
      <div id="ppmContent"></div>
      <div class="ppm-footer"><div class="ppm-info"><span class="ppm-info-icon">ⓘ</span><div><strong id="ppmFooterCount"></strong><small>Mis à jour le ${esc(updated)}</small></div></div><div class="ppm-footer-actions"><button type="button" class="btn ppm-print" id="ppmPrint">▣ Imprimer</button><button type="button" class="btn ppm-close-main" onclick="closeModal()">Fermer</button></div></div>
    </div></div>`);

  const content=$("#ppmContent"),tabs=[...document.querySelectorAll(".ppm-tab")],footerCount=$("#ppmFooterCount");
  const slice=(items,view)=>{const size=pageSize(),pages=Math.max(1,Math.ceil(items.length/size));page[view]=Math.min(Math.max(1,page[view]),pages);const from=(page[view]-1)*size;return {rows:items.slice(from,from+size),pages,size}};
  const pager=(view,pages)=>pages<=1?"":`<div class="ppm-pagination"><button type="button" data-page="prev" ${page[view]<=1?"disabled":""}>‹</button><span>Page ${page[view]} / ${pages}</span><button type="button" data-page="next" ${page[view]>=pages?"disabled":""}>›</button></div>`;
  const bindPager=(view,pages)=>{document.querySelectorAll(".ppm-pagination button").forEach(b=>b.onclick=()=>{page[view]+=b.dataset.page==="next"?1:-1;show(view)})};

  const show=view=>{
    tabs.forEach(b=>b.classList.toggle("active",b.dataset.view===view));
    if(view==="trades"){
      const pg=slice(trades,view);footerCount.textContent=`${trades.length} métier${trades.length>1?"s":""} au total`;
      content.innerHTML=`<div class="ppm-section-head"><div><h3>Métiers du projet</h3><p>Consultez et gérez les métiers affectés à ce projet.</p></div><button id="ppmAddTrade" class="btn ppm-primary">+ Ajouter un métier</button></div>
      <div class="ppm-table-wrap"><table class="ppm-table ppm-table-trades"><thead><tr><th>MÉTIER</th><th>PHASE</th><th>MAIN-D'ŒUVRE</th><th>ACTIONS</th></tr></thead><tbody>${pg.rows.length?pg.rows.map(t=>{const mt=laborRows.filter(l=>l.trade_id===t.id).reduce((a,x)=>a+Number(x.amount||0),0),phase=t.phase||phaseForTrade(t.name);return `<tr><td><div class="ppm-trade-cell"><span class="ppm-trade-icon">${v29TradeIcon(t.name)}</span><strong>${esc(t.name)}</strong></div></td><td><span class="ppm-phase ${phase==="01_acquisition_etudes"?"blue":""}">${esc(phaseShort(phase))}</span></td><td class="ppm-money">${cash(mt)}</td><td><div class="ppm-row-actions"><button type="button" class="ppm-icon-btn ppm-view-trade" data-id="${t.id}" title="Voir">◉</button><button type="button" class="ppm-icon-btn ppm-edit-trade" data-id="${t.id}" title="Modifier">✎</button>${S.session.user.role==="admin"?`<button type="button" class="ppm-icon-btn danger ppm-delete-trade" data-id="${t.id}" title="Supprimer">♜</button>`:""}</div></td></tr>`}).join(""):`<tr><td colspan="4" class="empty">Aucun métier affecté à ce projet</td></tr>`}</tbody></table></div>${pager(view,pg.pages)}`;
      $("#ppmAddTrade").onclick=()=>modal(`<h2>Ajouter un métier · ${esc(p.name)}</h2><form id="v29TradeForm" data-project="${projectId}"><label>Phase<select name="phase">${TRADE_PHASES.map(ph=>`<option value="${esc(ph.key)}">${esc(ph.label)}</option>`).join("")}</select></label><label>Métier<input name="name" required></label><label>Description<textarea name="description"></textarea></label><button class="btn primary full">Ajouter</button></form>`);
      document.querySelectorAll(".ppm-view-trade").forEach(b=>b.onclick=()=>{const t=trades.find(x=>x.id===b.dataset.id);if(!t)return;const labor=laborRows.filter(x=>x.trade_id===t.id);modal(`<h2>${esc(t.name)}</h2><p><strong>Phase :</strong> ${esc(phaseLabel(t.phase||phaseForTrade(t.name)))}</p><p><strong>Description :</strong> ${esc(t.description||"—")}</p><h3>Main-d'œuvre</h3>${table(["Date","Prestataire","Travaux","Montant"],labor.map(x=>`<tr><td>${df(x.expense_date)}</td><td>${esc(x.worker_name||"")}</td><td>${esc(x.description||"")}</td><td class="money">${cash(x.amount)}</td></tr>`))}`)});
      document.querySelectorAll(".ppm-edit-trade").forEach(b=>b.onclick=()=>{const t=trades.find(x=>x.id===b.dataset.id);if(!t)return;modal(`<h2>Modifier le métier</h2><form id="v29TradeEditForm" data-id="${t.id}" data-project="${projectId}"><label>Phase<select name="phase">${TRADE_PHASES.map(ph=>`<option value="${esc(ph.key)}" ${ph.key===(t.phase||phaseForTrade(t.name))?"selected":""}>${esc(ph.label)}</option>`).join("")}</select></label><label>Métier<input name="name" value="${esc(t.name)}" required></label><label>Description<textarea name="description">${esc(t.description||"")}</textarea></label><button class="btn primary full">Enregistrer</button></form>`)});
      document.querySelectorAll(".ppm-delete-trade").forEach(b=>b.onclick=()=>confirmBox("Supprimer ce métier ?",async()=>{await post("/api/save",{entity:"trade",action:"delete",record:{id:b.dataset.id}});await reload();closeModal();v32ProjectHub(projectId,"trades");toast("Métier supprimé")}));
      bindPager(view,pg.pages);
    }
    if(view==="expenses"){
      const pg=slice(expenses,view);footerCount.textContent=`${expenses.length} dépense${expenses.length>1?"s":""} enregistrée${expenses.length>1?"s":""}`;
      content.innerHTML=`<div class="ppm-section-head"><div><h3>Dépenses du projet</h3><p>Consultez les dépenses affectées aux différents métiers.</p></div><button id="ppmAddExpense" class="btn ppm-primary">+ Ajouter une dépense</button></div>
      <div class="ppm-table-wrap"><table class="ppm-table ppm-table-expenses"><thead><tr><th>DATE</th><th>MÉTIER</th><th>DÉSIGNATION</th><th>FOURNISSEUR</th><th>MONTANT</th><th>ACTIONS</th></tr></thead><tbody>${pg.rows.length?pg.rows.map(x=>`<tr><td>${df(x.expense_date)}</td><td>${esc(x.trade_name||"—")}</td><td><strong>${esc(x.description||"")}</strong></td><td>${esc(x.supplier_name||"—")}</td><td class="ppm-money">${cash(x.total_price)}</td><td>${S.session.user.role==="admin"?`<button type="button" class="ppm-icon-btn danger ppm-delete-expense" data-id="${x.id}" title="Supprimer">♜</button>`:""}</td></tr>`).join(""):`<tr><td colspan="6" class="empty">Aucune dépense enregistrée</td></tr>`}</tbody></table></div>${pager(view,pg.pages)}`;
      $("#ppmAddExpense").onclick=()=>modal(`<h2>Nouvelle dépense · ${esc(p.name)}</h2><form id="v29ExpenseForm" data-project="${projectId}"><label>Métier<select name="trade_id">${opts(trades)}</select></label><label>Fournisseur<select name="supplier_id">${opts(S.data.suppliers||[])}</select></label><label>Date<input name="expense_date" type="date" value="${new Date().toISOString().slice(0,10)}"></label><label>Désignation<input name="description" required></label><label>Quantité<input name="quantity" type="number" step=".01" value="1"></label><label>Unité<input name="unit"></label><label>Prix unitaire<input name="unit_price" type="number" min="0"></label><label>Référence<input name="reference"></label><button class="btn primary full">Enregistrer</button></form>`);
      document.querySelectorAll(".ppm-delete-expense").forEach(b=>b.onclick=()=>confirmBox("Supprimer cette dépense du projet ?",async()=>{await post("/api/save",{entity:"expense",action:"delete",record:{id:b.dataset.id}});await reload();closeModal();v32ProjectHub(projectId,"expenses");toast("Dépense supprimée")}));
      bindPager(view,pg.pages);
    }
    if(view==="suppliers"){
      const pg=slice(suppliers,view);footerCount.textContent=`${suppliers.length} fournisseur${suppliers.length>1?"s":""} affecté${suppliers.length>1?"s":""}`;
      content.innerHTML=`<div class="ppm-section-head"><div><h3>Fournisseurs du projet</h3><p>Consultez et affectez les fournisseurs intervenant sur ce projet.</p></div><button id="ppmAddSupplier" class="btn ppm-primary">+ Affecter un fournisseur</button></div>
      <div class="ppm-table-wrap"><table class="ppm-table ppm-table-suppliers"><thead><tr><th>FOURNISSEUR</th><th>CONTACT</th><th>SPÉCIALITÉ</th><th>DÉPENSES</th><th>ACTIONS</th></tr></thead><tbody>${pg.rows.length?pg.rows.map(x=>{const spent=expenses.filter(e=>e.supplier_id===x.supplier_id).reduce((a,e)=>a+Number(e.total_price||0),0);return `<tr><td><strong>${esc(x.supplier_name)}</strong></td><td>${esc(x.phone||"—")}</td><td>${esc(x.specialty||"—")}</td><td class="ppm-money">${cash(spent)}</td><td>${S.session.user.role==="admin"?`<button type="button" class="ppm-icon-btn danger ppm-delete-supplier" data-id="${x.id}" title="Retirer du projet">♜</button>`:""}</td></tr>`}).join(""):`<tr><td colspan="5" class="empty">Aucun fournisseur affecté</td></tr>`}</tbody></table></div>${pager(view,pg.pages)}`;
      $("#ppmAddSupplier").onclick=()=>modal(`<h2>Affecter un fournisseur · ${esc(p.name)}</h2><form id="v29SupplierForm" data-project="${projectId}"><label>Fournisseur<select name="supplier_id" required>${opts(S.data.suppliers||[])}</select></label><label>Notes<textarea name="notes"></textarea></label><button class="btn primary full">Affecter</button></form>`);
      document.querySelectorAll(".ppm-delete-supplier").forEach(b=>b.onclick=()=>confirmBox("Retirer ce fournisseur du projet ?",async()=>{await post("/api/save",{entity:"project_supplier",action:"delete",record:{id:b.dataset.id,project_id:projectId}});await reload();closeModal();v32ProjectHub(projectId,"suppliers");toast("Fournisseur retiré du projet")}));
      bindPager(view,pg.pages);
    }
  };
  tabs.forEach(b=>b.onclick=()=>show(b.dataset.view));$("#ppmPrint").onclick=()=>window.print();show(initialView);
}
...
function projects(){
  ...
  document.querySelectorAll('.v28-project-row').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;v32ProjectHub(r.dataset.id)});
...
document.addEventListener("submit",async e=>{
  ...
  try{
    if(f.id==="v29TradeForm"){
      ...
      await reload();closeModal();v32ProjectHub(f.dataset.project);toast("Métier ajouté");
      ...
      await reload();closeModal();v32ProjectHub(f.dataset.project);toast("Métier modifié");
    }else if(f.id==="v29ExpenseForm"){
      await post("/api/save",{entity:"expense",action:"create",record:{project_id:f.dataset.project,...fd(f)}});
      await reload();closeModal();v32ProjectHub(f.dataset.project);toast("Dépense enregistrée");
      ...
      await reload();closeModal();v32ProjectHub(f.dataset.project);toast("Fournisseur affecté");
    }
