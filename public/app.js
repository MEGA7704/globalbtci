...
function nav(){
  ...
  const n=superA?[["home","Accueil"],["super","Tableau de bord"],["companies","Entreprises"],["members","Membres"],["subscriptions","Abonnements"],["resets","Mots de passe"],["audit","Journal"]]:[["home","Accueil"],["dashboard","Tableau de bord"],["projects","Projets"],...(admin?[["users","Utilisateurs"]]:[]),["settings","Paramètres"]];
...
// V58 : liste des projets compacte, statut en popup et actions limitées.
function projects(){
  ...
  const rowHtml=x=>`<tr class="clickable-row v28-project-row ${Number(x.locked)===1?"v57-project-locked":""}" data-id="${x.id}" title="${Number(x.locked)===1?"Projet verrouillé — déverrouillage obligatoire":"Ouvrir le projet"}">
    ...
    <td class="center">${admin?`<button type="button" class="status v58-status-button v28Status" data-id="${x.id}" title="Modifier le statut">${esc(statusLabel(x.status))}</button>`:`<span class="status">${esc(statusLabel(x.status))}</span>`}</td>
    <td class="v41-project-actions-cell">${admin?`<div class="project-actions v28-actions"><button type="button" class="btn small secondary v28Edit" data-id="${x.id}">Modifier</button><button type="button" class="btn small secondary v28Lock" data-id="${x.id}" data-act="${Number(x.locked)?'unlock':'lock'}">${Number(x.locked)?'Déverrouiller':'Verrouiller'}</button><button type="button" class="btn small danger v28Delete" data-id="${x.id}">Supprimer</button></div>`:`<span class="muted">—</span>`}</td>
  </tr>`;
